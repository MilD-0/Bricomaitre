...
map \$arg_sf_variant \$storefront_variant_query {
  default '';
  new new;
  old legacy;
  legacy legacy;
...
map \$cookie_bric_sf_variant \$storefront_variant_cookie {
  default '';
  new new;
  old legacy;
  legacy legacy;
...
map "\$storefront_variant_query:\$storefront_variant_cookie" \$storefront_variant {
  default new;
  "~^new:" new;
  "~^legacy:" legacy;
  "~^old:" legacy;
...
map \$storefront_variant \$storefront_upstream {
  default storefront-${slot}:3002;
  legacy storefront-old-${slot}:3002;
}
...
server {
  ...
  location / {
    ...
    proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
    proxy_set_header X-Forwarded-Proto \$scheme;
    proxy_set_header X-Bric-Storefront-Variant \$storefront_variant;
    proxy_set_header Upgrade \$http_upgrade;
    proxy_set_header Connection \$connection_upgrade;
  }
}
