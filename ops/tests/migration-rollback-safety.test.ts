...
describe('migration rollback-safety verification', () => {
  ...
  it('bridges the source-free historical release through its pinned baseline', () => {
    ...
    const migrations = join(workspaceRoot, 'apps/admin/drizzle/migrations');
    const sourceMigrations = join(source, 'apps/admin/drizzle/migrations');
    const journal = JSON.parse(readFileSync(join(migrations, 'meta/_journal.json'), 'utf8'));
    // This bridge promises compatibility with the pinned 88-migration baseline,
    // independently of whether later releases introduce a contract migration.
    journal.entries = journal.entries.slice(0, 88);
    for (const entry of journal.entries) {
      const filename = `${entry.tag}.sql`;
      writeFileSync(join(sourceMigrations, filename), readFileSync(join(migrations, filename)));
    }
    const additiveTag = '0088_bridge_additive_fixture';
    journal.entries.push({ idx: 88, tag: additiveTag });
    writeFileSync(join(sourceMigrations, `${additiveTag}.sql`), 'SELECT 1;');
    mkdirSync(join(sourceMigrations, 'meta'), { recursive: true });
    writeFileSync(join(sourceMigrations, 'meta/_journal.json'), JSON.stringify(journal));
    const baselineCommit = 'ab11790b49df28cdf99bde05052993962ad879f2';
    for (const directory of [previous, candidate]) {
      writeFileSync(
        join(directory, '.bric-release.env'),
        `BRIC_RELEASE_COMMIT=${baselineCommit}\n`,
      );
    }
    const build = spawnSync(
      'python3',
      [verifier, '--build-state', source, baselineCommit, join(candidate, '.bric-migrations.json')],
      { encoding: 'utf8' },
    );
  ...
  it('rejects the analytics calendar and catalog contract migration for automatic rollback', () => {
    const filename = '0106_align_analytics_reporting_contracts.sql';
    const sql = readFileSync(
      join(workspaceRoot, 'apps/admin/drizzle/migrations', filename),
      'utf8',
    );
    const previous = release({ '0000_base.sql': 'SELECT 1;' });
    const candidate = release({ '0000_base.sql': 'SELECT 1;', [filename]: sql });

    const result = run(previous, candidate);

    expect(result.status).toBe(1);
    expect(result.stderr).toContain(filename);
    expect(result.stderr).toContain('drop a column');
    expect(result.stderr).toContain('create a unique index');
  });
});
