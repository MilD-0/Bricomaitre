...
test('renders a fast, focused, accessible single-product campaign in French', async ({ page }) => {
  ...
  page.on('console', (message) => {
    ...
    if (message.type() === 'error' && (text.includes('hydration') || text.includes('hydrated') || text.includes('server rendered html'))) hydrationErrors.push(message.text());
  });
  ...
  await expect(page.getByRole('link', { name: 'Promos' })).toHaveAttribute('href', '/fr/products?discounted=1');
  await expect(page.locator('.site-footer')).toBeVisible();
  await expect(page.getByRole('heading', { level: 2, name: 'Finaliser votre commande' })).toBeVisible();
...
test('keeps the campaign usable in Arabic on a narrow phone', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1, name: 'أنِر كل مشروع' })).toBeVisible();
