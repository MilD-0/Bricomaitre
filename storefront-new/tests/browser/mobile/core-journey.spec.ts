...
async function swipeCarousel(page: Page, context: BrowserContext, selector: string) {
  ...
  const before = await viewport.locator(':scope > div').evaluate((element) => getComputedStyle(element).transform);
...
test('supports touch navigation, search, and homepage carousels', async ({ page, context }) => {
  ...
  expect(await drawerLocaleToggle.evaluate((element) => getComputedStyle(element.querySelector('.navigation-locale-indicator')!).transitionProperty)).toContain('transform');
  const drawerHeaderGeometry = await menu.locator('.mobile-sheet-header').evaluate((header) => {
    ...
    const toggle = header.querySelector<HTMLElement>('.navigation-drawer-locale-toggle')!.getBoundingClientRect();
  ...
  await expect(menu.locator('#global-search-results-mobile-navigation')).toBeVisible();
  await expect(menu.locator('[data-search-result]').first()).toBeVisible();
  await search.fill('');
  ...
  const productCardHeights = await page.locator('.home-product-carousel').evaluateAll((carousels) => carousels.map((carousel) => (
    [...carousel.querySelectorAll<HTMLElement>('.catalog-card')].map((card) => Math.round(card.getBoundingClientRect().height))
  )));
...
test('completes the essential product and checkout journey by touch', async ({ page }) => {
  ...
  const zoomTriggerGeometry = await zoomTrigger.locator('.product-media-zoom-label').evaluate((label) => {
  ...
  await page.getByRole('button', { name: 'Ouvrir le menu' }).tap();
  const menu = page.getByRole('dialog', { name: 'Ouvrir le menu' });
  await expect(menu.getByRole('link', { name: 'Passer la commande' })).toHaveAttribute('href', '/fr/checkout');
  await menu.getByRole('button', { name: 'Fermer le menu' }).tap();
