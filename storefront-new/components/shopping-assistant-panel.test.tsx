...
vi.mock('@/lib/analytics', () => ({
  getAnalyticsIdentity: () => ({ visitId: 'visit-1', journeyId: 'journey-1', sessionId: 'session-1' }),
...
const labels: ShoppingAssistantLabels = {
  open: 'Trouver le bon outil', title: 'Conseiller produits', close: 'Fermer', liveCatalog: 'Catalogue en direct', newChat: 'Nouvelle discussion',
...
describe('ShoppingAssistantPanel', () => {
  ...
  it('submits a bounded conversation and renders grounded product results', async () => {
    ...
    await waitFor(() => expect(JSON.parse(localStorage.getItem('bricomaitre-shopping-assistant-chat-v1:fr') ?? '[]')).toHaveLength(2));
  });
  ...
  it('renders the first assistant text chunk before the response finishes', async () => {
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        controller.enqueue(encoder.encode('{"type":"status","status":"thinking"}\n{"type":"text-delta","delta":"Première réponse"}\n'));
      },
    });
    ...
    await act(async () => {
      streamController!.enqueue(encoder.encode('{"type":"text-delta","delta":" immédiate."}\n{"type":"result","mode":"ai","products":[]}\n'));
  ...
  it('restores local history and starts a new chat on request', async () => {
    localStorage.setItem('bricomaitre-shopping-assistant-chat-v1:fr', JSON.stringify([
    ...
    await waitFor(() => expect(localStorage.getItem('bricomaitre-shopping-assistant-chat-v1:fr')).toBeNull());
  });
