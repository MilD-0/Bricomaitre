...
const TRUCK_VARIANTS: Variants = {
  ...
  animate: {
    y: [0, -1, 0, -0.5, 0],
    transition: { duration: 0.4, ease: "easeInOut", repeat: Number.POSITIVE_INFINITY, repeatType: "loop" },
  },
};
...
const SPEED_LINE_VARIANTS: Variants = {
  ...
  animate: (custom: number) => ({
    ...
    transition: { duration: 0.5, ease: "easeOut", repeat: Number.POSITIVE_INFINITY, delay: custom * 0.08, times: [0, 0.2, 0.6, 1] },
  }),
};
    ...
    return (
      <div className={className} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} {...props}>
        <svg className="overflow-visible" fill="none" height={size} stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" width={size} xmlns="http://www.w3.org/2000/svg">
          {[{ y: 8, width: 5, x: 0 }, { y: 11, width: 7, x: -1 }, { y: 14, width: 4, x: 0 }].map((line, index) => (
            <motion.line animate={controls} custom={index} initial="normal" key={`speed-${index}`} strokeLinecap="round" strokeWidth="2" variants={SPEED_LINE_VARIANTS} x1={line.x} x2={line.x + line.width} y1={line.y} y2={line.y} />
          ))}
