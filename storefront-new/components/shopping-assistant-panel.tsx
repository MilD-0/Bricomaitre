...
function storedEntries(value: unknown): ChatEntry[] {
  ...
  return value.slice(-20).flatMap((entry): ChatEntry[] => {
    ...
    if (typeof candidate.id !== 'string'
      ...
      || (candidate.products !== undefined && (!Array.isArray(candidate.products) || !candidate.products.every(isStoredProduct)))) return [];
...
export function ShoppingAssistantPanel({
  ...
  useEffect(() => {
    ...
    const storageKey = `${chatStoragePrefix}:${locale}`;
    try {
      if (messages.length > 0) localStorage.setItem(storageKey, JSON.stringify(messages.slice(-20)));
  ...
  async function sendMessage(content: string) {
    ...
    void trackNavigationEvent({
      ...
      locale,
      metadata: { surface: 'ai_assistant', target: 'submitted', intent: classifyShoppingAssistantIntent(normalized) },
    });
    ...
    try {
      ...
      const response = await fetch('/api/ai/chat', {
        ...
        body: JSON.stringify({
          locale,
          telemetry: identity.journeyId && identity.sessionId ? {
      ...
      await consumeShoppingAssistantResponse(response, {
        onTextDelta(delta) {
          ...
          setMessages((current) => {
            ...
            if (existing < 0) return [...current, { id: assistantId, role: 'assistant', content: delta }];
            return current.map((message, index) => index === existing ? { ...message, content: `${message.content}${delta}` } : message);
          });
        },
        onResult(result) {
    ...
    <MobileSheet
      ...
      headerAction={(
        <div className="shopping-assistant-header-actions">
          <span className="shopping-assistant-live"><span aria-hidden="true" />{labels.liveCatalog}</span>
          <button
            type="button"
