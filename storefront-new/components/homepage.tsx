import type { StorefrontHomepageResponse, StorefrontSettingsResponse } from '@bric/storefront-core/contracts';
...
const copy = {
  fr: { top: 'Top produits', categories: 'Acheter par catégorie', featured: 'Nos sélections', brands: 'Nos marques', cards: 'Bien choisir pour mieux travailler', add: 'Ajouter au panier', view: 'Voir le produit', delivery: 'Livraison rapide partout en Algérie', payment: 'Paiement à la livraison', unavailable: 'Nos produits sont momentanément indisponibles. Veuillez réessayer dans quelques instants.' },
  ar: { top: 'أفضل المنتجات', categories: 'تسوق حسب الفئة', featured: 'اختياراتنا', brands: 'علاماتنا', cards: 'الاختيار الصحيح لعمل أفضل', add: 'أضف إلى السلة', view: 'عرض المنتج', delivery: 'توصيل سريع إلى كل ولايات الجزائر', payment: 'الدفع عند الاستلام', unavailable: 'منتجاتنا غير متاحة مؤقتًا. يرجى المحاولة مرة أخرى بعد قليل.' },
} as const;
...
function TrustSignals({ locale, contact }: { locale: Locale; contact: StorefrontSettingsResponse }) {
  const text = copy[locale];
  return <section className="home-trust-band" aria-label="Services"><ul><ProductTrustSignal icon="delivery"><strong>{text.delivery}</strong></ProductTrustSignal><ProductTrustSignal icon="payment"><strong>{text.payment}</strong></ProductTrustSignal><ProductTrustSignal icon="confirmation"><strong dir="ltr">{contact.phoneDisplay}</strong></ProductTrustSignal></ul></section>;
}
