...
function exceptionLabel(variant: CustomerOrderTrackingVariant, labels: TrackingLabels) {
  return variant === 'delayed' || variant === 'cancelled' || variant === 'returned' || variant === 'failed'
    ? labels[variant]
    : null;
}

export function OrderTracking({ order, labels }: { order: StorefrontOrderResponseItem; labels: TrackingLabels }) {
  ...
  return (
    <section className="order-tracking" aria-labelledby="order-tracking-title">
      <header>
        <div>
          <span className="order-tracking-live"><CircleDot aria-hidden="true" />{labels.live}</span>
      ...
      {exception ? <p className={`order-tracking-alert is-${state.variant}`} role="status">{exception}</p> : null}
    </section>
  );
}
