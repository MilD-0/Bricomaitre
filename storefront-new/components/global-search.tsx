...
export function GlobalSearch({ locale, labels }: { locale: Locale; labels: GlobalSearchLabels }) {
  ...
  return (
    <div className="global-search" ref={rootRef}>
      <form action={`/${locale}/products`} method="get" role="search" onSubmit={() => {
        setOpen(false);
        void trackNavigationEvent({
          eventName: 'search',
          locale,
      ...
      {open ? (
        <div className="global-search-panel" id="global-search-results" aria-label={labels.results}>
                    ...
                    <a
                      ...
                      onClick={() => {
                        ...
                        void trackNavigationEvent({
                          ...
                          metadata: { surface: 'global_search', target: token, position: index + 1 },
                        });
                      }}
                    >
                      <span className="global-search-thumb">
                        {image ? <StorefrontImage src={image} alt="" width={56} height={56} sizes="56px" /> : <span aria-hidden="true">B</span>}
                      </span>
          ...
          <a className="global-search-all" href={`/${locale}/products${value.trim() ? `?q=${encodeURIComponent(value.trim())}` : ''}`}>
            {labels.viewAll}
