...
const mocks = vi.hoisted(() => ({ prepare: vi.fn(), trigger: vi.fn(), start: vi.fn(), stop: vi.fn() }));
...
vi.mock('@/components/ui/arrow-up-right', async () => {
  ...
  return { ArrowUpRightIcon: React.forwardRef((_props, ref) => {
    React.useImperativeHandle(ref, () => ({ startAnimation: mocks.start, stopAnimation: mocks.stop }));
...
describe('landing page interactions', () => {
  ...
  it('keeps a mobile conversion CTA available until the order form enters view', () => {
    ...
    render(<><div id="landing-order" /><LandingMobileCta href="#landing-order" label="Commander" price="4 500 DA" /></>);
