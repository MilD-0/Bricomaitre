...
describe('CatalogLiveSearch', () => {
  ...
  it('uses adaptive delays for intent, input length, and weak connections', () => {
    ...
    expect(getAdaptiveSearchDelay('per', 'insertText', { effectiveType: '2g', saveData: true })).toBe(640);
  });

  it('replaces the URL after the debounce while preserving filters and resetting pagination', () => {
    render(<CatalogLiveSearch initialValue="" label="Search" placeholder="Tool" searchingLabel="Searching" />);
    ...
    expect(navigation.replace).toHaveBeenCalledWith('/fr/products?brand=2&q=perc', { scroll: false });
  });

  it('waits for an Arabic composition session to finish before searching', () => {
    render(<CatalogLiveSearch initialValue="" label="البحث" placeholder="مثقاب" searchingLabel="جارٍ البحث" />);
    ...
    expect(navigation.replace).toHaveBeenCalledWith('/fr/products?brand=2&q=%D8%A7%D8%AF%D8%A7%D9%87', { scroll: false });
  });
});
