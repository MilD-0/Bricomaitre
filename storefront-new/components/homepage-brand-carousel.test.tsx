...
import { HomepageBrandCarousel, HomepageCategoryCarousel } from './homepage-carousels';
...
const brands = [
  { id: 1, name: 'WADFOW', slug: 'wadfow', image: '/wadfow.png', featured: true, createdAt: stamp, updatedAt: stamp },
  { id: 2, name: 'INGCO', slug: 'ingco', image: null, featured: false, createdAt: stamp, updatedAt: stamp },
  { id: 3, name: 'TOTAL', slug: 'total', image: '/total.png', featured: false, createdAt: stamp, updatedAt: stamp },
  { id: 4, name: 'TOLSEN', slug: 'tolsen', image: '/tolsen.png', featured: false, createdAt: stamp, updatedAt: stamp },
];
const category = { id: 3, name: 'Éclairage', slug: null, nameEn: null, nameAr: null, image: null, parentId: null, properties: [], featured: false, createdAt: stamp, updatedAt: stamp };
...
describe('HomepageBrandCarousel', () => {
  it('uses Embla loop mode with uninterrupted auto-scroll', () => {
    ...
    expect(container.querySelector('a[aria-label="WADFOW"]')).toHaveAttribute('href', '/fr/brands/wadfow');
  });

  it('keeps free-drag touch motion for category carousels', () => {
    const { container } = render(<HomepageCategoryCarousel categories={[{ ...category, slug: 'eclairage' }]} locale="fr" />);

    expect(mocks.embla).toHaveBeenLastCalledWith(expect.objectContaining({
      align: 'start',
      dragFree: true,
      direction: 'ltr',
    }));
    expect(container.querySelector('.home-category-carousel a')).toHaveAttribute('href', '/fr/categories/eclairage');
  });
});
