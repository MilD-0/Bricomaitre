...
describe('CartDrawer', () => {
  ...
  it('updates quantity in storage and reports the commerce interaction', () => {
    ...
    render(<CartDrawer locale="fr" items={[item]} labels={labels} onClose={vi.fn()} onItemsChange={onItemsChange} />);

    expect(screen.getByRole('dialog', { name: labels.title })).toBeVisible();
    expect(screen.getByRole('dialog', { name: labels.title }).parentElement?.parentElement).toBe(document.body);
    fireEvent.pointerDown(screen.getByRole('button', { name: `${labels.increase}: ${item.title}` }));
  ...
  it('removes an item and closes with Escape', () => {
    const onClose = vi.fn();
    const onItemsChange = vi.fn();
    render(<CartDrawer locale="fr" items={[item]} labels={labels} onClose={onClose} onItemsChange={onItemsChange} />);
    ...
    expect(analytics).toHaveBeenCalledWith(expect.objectContaining({ eventName: 'remove_from_cart', quantity: 1, value: 4500 }));
  ...
  it('shows a useful empty state', () => {
    render(<CartDrawer locale="ar" items={[]} labels={labels} onClose={vi.fn()} onItemsChange={vi.fn()} />);
    expect(screen.getByRole('heading', { name: labels.emptyTitle })).toBeVisible();
    expect(screen.getByRole('link', { name: labels.continueShopping })).toHaveAttribute('href', '/ar/products');
  });
});
