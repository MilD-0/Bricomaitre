...
import { BrandPageContent } from './page';
...
describe('brand landing route', () => {
  ...
  it('renders brand-specific semantics with the canonical brand filter', async () => {
    ...
    expect(mocks.catalog.mock.calls[0][0]).toEqual(expect.objectContaining({ heading: expect.objectContaining({ title: 'Wadfow' }) }));
    await expect(mocks.catalog.mock.calls[0][0].searchParams).resolves.toEqual({ q: 'perceuse', brand: '8' });
  });
  ...
  it('does not convert a metadata outage into a missing brand', async () => {
    ...
    expect(mocks.capture).toHaveBeenCalledWith(error, { locale: 'ar', operation: 'brand-resolution' });
  });
});
