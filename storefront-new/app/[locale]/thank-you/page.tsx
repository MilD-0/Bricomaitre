...
export async function ThankYouPageContent({ params, searchParams }: ThankYouPageProps) {
  ...
  const [t, contact, initialOrder] = await Promise.all([
    ...
    validToken
      ? fetchStorefrontOrderByToken(validToken).catch(() => null)
  ...
  return (
    <PageShell locale={locale}>
      <ThankYouConfirmation
        ...
        token={validToken}
        initialConfirmation={initialOrder ? {
          order: initialOrder,
          cartMode: 'cart',
          stateName: null,
        ...
        labels={{
          ...
          trackingTitle: t('trackingTitle'), trackingLive: t('trackingLive'), trackingWaiting: t('trackingWaiting'),
          trackingPreparing: t('trackingPreparing'), trackingOnWay: t('trackingOnWay'), trackingDelivered: t('trackingDelivered'),
          trackingDelayed: t('trackingDelayed'), trackingCancelled: t('trackingCancelled'),
          trackingReturned: t('trackingReturned'), trackingFailed: t('trackingFailed'),
        }}
