...
describe('POST /api/marketing/meta', () => {
  ...
  it('forwards attribution headers and the governed body to storefront-api', async () => {
    vi.mocked(fetch).mockResolvedValue(new Response('{"ok":true}', { status: 202, headers: { 'content-type': 'application/json' } }));
    const body = JSON.stringify({ eventId: 'event-1', eventName: 'PageView' });
    const response = await POST(new NextRequest('http://localhost/api/marketing/meta', {
      method: 'POST',
      body,
      headers: { cookie: '_fbp=fb.1.1.1', 'user-agent': 'phone', 'x-forwarded-for': '203.0.113.4' },
    }));
  ...
  it('fails non-fatally when the optional destination path is unavailable', async () => {
    ...
    const response = await POST(new NextRequest('http://localhost/api/marketing/meta', { method: 'POST', body: '{}' }));
