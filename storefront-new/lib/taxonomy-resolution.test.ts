...
describe('taxonomy resolution', () => {
  it('distinguishes a matching slug from a missing taxonomy entry', async () => {
    ...
    await expect(resolveBrandSlug('missing', getCatalogMeta)).resolves.toEqual({ status: 'missing' });
  });
  ...
  it('keeps an upstream outage distinct from a missing slug', async () => {
    ...
    await expect(resolveCategorySlug('eclairage', getCatalogMeta)).resolves.toEqual({ status: 'unavailable', error });
  });
});
