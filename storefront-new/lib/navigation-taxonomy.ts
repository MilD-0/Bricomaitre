...
export function buildNavigationTaxonomy(categories: NavigationTaxonomyCategory[]): NavigationTaxonomyNode[] {
  const categoryById = new Map(categories.map((category) => [category.id, category]));
  const childrenByParent = new Map<number, NavigationTaxonomyCategory[]>();

  for (const category of categories) {
    if (category.parentId === null || category.parentId === category.id || !categoryById.has(category.parentId)) continue;
    const children = childrenByParent.get(category.parentId) ?? [];
    children.push(category);
    childrenByParent.set(category.parentId, children);
  }

  const materialize = (category: NavigationTaxonomyCategory, ancestors: ReadonlySet<number>): NavigationTaxonomyNode => {
