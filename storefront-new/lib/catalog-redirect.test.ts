...
describe('legacy catalog taxonomy redirects', () => {
  it('resolves single ID filters to canonical slug paths', async () => {
    ...
    await expect(resolveLegacyTaxonomyRedirect('fr', { category: '4' }, getCatalogMeta)).resolves.toBe('/fr/categories/eclairage');
    await expect(resolveLegacyTaxonomyRedirect('ar', { brand: '8' }, getCatalogMeta)).resolves.toBe('/ar/brands/wadfow');
  });

  it('does not redirect compound discovery queries or metadata outages', async () => {
    const getCatalogMeta = vi.fn();
    await expect(resolveLegacyTaxonomyRedirect('fr', { category: '4', q: 'lampe' }, getCatalogMeta)).resolves.toBeNull();
    expect(getCatalogMeta).not.toHaveBeenCalled();
    getCatalogMeta.mockRejectedValue(new Error('unavailable'));
    await expect(resolveLegacyTaxonomyRedirect('fr', { category: '4' }, getCatalogMeta)).resolves.toBeNull();
  });
});
