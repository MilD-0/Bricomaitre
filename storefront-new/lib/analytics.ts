...
function buildAnalyticsPayload(
  ...
  return {
    ...
    metadata: {
      ...
      viewportClass: window.innerWidth < 480 ? 'small_phone' : window.innerWidth < 768 ? 'mobile' : 'desktop',
...
export async function trackCheckoutEvent(input: CheckoutAnalyticsEventInput, pageType: 'checkout' | 'thank_you' = 'checkout') {
  if (typeof window === 'undefined') return null;
  let payload;
  try {
