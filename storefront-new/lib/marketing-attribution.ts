...
const storefrontAttributionSchema = z.object({
...
function readStorefrontAttribution(now: number) {
  try {
    const parsed = storefrontAttributionSchema.safeParse(JSON.parse(window.localStorage.getItem(STOREFRONT_ATTRIBUTION_KEY) ?? 'null'));
    if (parsed.success && now - parsed.data.capturedAt <= ATTRIBUTION_MAX_AGE_SECONDS * 1000) return parsed.data;
  } catch {
...
export function captureStorefrontAttribution(now = Date.now()): StorefrontAttribution {
  ...
  const next = storefrontAttributionSchema.parse(startsNewVisit || !stored ? {
    visitId: createId(),
