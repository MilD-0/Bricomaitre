...
export function getLandingPageUrl(locale: Locale, slug: string) {
...
export function buildLandingPageMetadata(
  ...
  const imageUrl = [hero?.imageUrl, ...page.product.media.map((media) => media.url)]
    .find((url): url is string => Boolean(url && isSafeProductImageUrl(url)));
  const imageAlt = hero?.imageAlt || (locale === 'ar' && page.product.titleAr ? page.product.titleAr : page.product.title);
  ...
  return {
    ...
    openGraph: {
      ...
      alternateLocale: hasAlternateLocale ? [alternateLocale === 'ar' ? 'ar_DZ' : 'fr_DZ'] : undefined,
      images,
    },
    twitter: {
