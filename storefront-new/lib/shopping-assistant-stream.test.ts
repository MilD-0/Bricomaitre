...
const product = { id: 1, token: 'drill', title: 'Perceuse', titleAr: null, description: null, descriptionAr: null, price: '5000.00', oldPrice: null, inStock: true, availabilityStatus: 'in_stock', imageUrl: null, brand: null, category: null };
  ...
  it('delivers text incrementally before the final product result', async () => {
    ...
    const response = new Response(new ReadableStream({
      start(controller) {
        ...
        controller.enqueue(encoder.encode(`{"type":"text-delta","delta":"une option."}\n${JSON.stringify({ type: 'result', mode: 'ai', products: [product] })}\n`));
        controller.close();
      },
    ...
    await consumeShoppingAssistantResponse(response, { onTextDelta: (delta) => deltas.push(delta), onResult });
