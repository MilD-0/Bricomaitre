...
const env = {
  ...
  NEXT_PUBLIC_STOREFRONT_IMAGE_ORIGINS: 'https://media.example.com, invalid,https://cdn.example.com',
};
...
describe('product image policy', () => {
  ...
  it('displays canonical legacy HTTPS images without trusting them for optimization or metadata', () => {
    expect(isDisplayableProductImageUrl('https://legacy-media.example.com/catalog/tool.jpg', env)).toBe(true);
    expect(isSafeProductImageUrl('https://legacy-media.example.com/catalog/tool.jpg', env)).toBe(false);
    expect(isDisplayableProductImageUrl('http://legacy-media.example.com/catalog/tool.jpg', env)).toBe(false);
    expect(isDisplayableProductImageUrl('https://user:secret@legacy-media.example.com/tool.jpg', env)).toBe(false);
