...
export type ShoppingAssistantIntent = 'product_search' | 'product_comparison' | 'compatibility' | 'price' | 'availability' | 'how_to' | 'recommendation' | 'other';

const catalogSearchStopWords = new Set([
  'a', 'ai', 'au', 'aux', 'avec', 'ce', 'cette', 'ces', 'cherche', 'chercher', 'combien', 'compare', 'courte', 'dans', 'de', 'des', 'disponible', 'disponibles', 'du', 'en', 'est', 'et', 'je', 'la', 'le', 'les', 'me', 'moi', 'montre', 'pour', 'produit', 'produits', 'recommande', 'reponse', 'réponse', 'stock', 'tres', 'très', 'trouve', 'trouve-moi', 'un', 'une', 'outil', 'outils', 'veux',
  'أبحث', 'أريد', 'اعرض', 'الأداة', 'الأدوات', 'السعر', 'عن', 'في', 'لي', 'منتج', 'منتجات', 'متوفر', 'متوفرة', 'مع',
]);

export function catalogSearchQuery(value: string) {
  const tokens = (value.normalize('NFKC').match(/[\p{L}\p{N}][\p{L}\p{N}+.-]*/gu) ?? [])
...
export function classifyShoppingAssistantIntent(value: string): ShoppingAssistantIntent {
  ...
  if (includesAny('compare', 'compar', 'versus', 'vs', 'الفرق', 'قارن', 'مقارنة')) return 'product_comparison';
  if (includesAny('compatible', 'compatib', 'fit ', 'works with', 'يركب', 'متوافق', 'يناسب')) return 'compatibility';
  ...
  if (includesAny('how to', 'comment', 'utiliser', 'usage', 'طريقة', 'كيف', 'استعمال')) return 'how_to';
  if (includesAny('recommend', 'conseil', 'meilleur', 'choisir', 'اقترح', 'أفضل', 'أنصح')) return 'recommendation';
  if (includesAny('find', 'search', 'cherche', 'besoin', 'أبحث', 'ابحث', 'أريد')) return 'product_search';
  return 'other';
}
