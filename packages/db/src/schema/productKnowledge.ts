...
export const productAttributeDataTypeEnum = pgEnum('product_attribute_data_type', [
  'text', 'number', 'boolean', 'enum', 'multi_enum',
]);
...
export const productProjectSuitabilityEnum = pgEnum('product_project_suitability', [
  'recommended', 'suitable', 'conditional', 'unsuitable',
]);
export const productEvidenceTypeEnum = pgEnum('product_evidence_type', [
  'manufacturer_document', 'manufacturer_page', 'admin_note', 'sales_data', 'other',
]);
...
export const productAttributeDefinitions = pgTable(
  ...
  (t) => [
    uniqueIndex('product_attribute_definitions_key_category_unique').on(t.key, sql`coalesce(${t.categoryId}, 0)`),
...
export const productAttributeValues = pgTable(
  'product_attribute_values',
  {
    ...
    definitionId: bigint('definition_id', { mode: 'number' }).notNull().references(() => productAttributeDefinitions.id, { onDelete: 'cascade' }),
  ...
  (t) => [
    uniqueIndex('product_attribute_values_product_definition_unique').on(t.productId, t.definitionId),
    ...
    check('product_attribute_values_confidence_check', sql`${t.confidence} is null or (${t.confidence} >= 0 and ${t.confidence} <= 1)`),
  ],
);
...
export const projectTypes = pgTable(
  ...
  (t) => [
    ...
    foreignKey({ columns: [t.parentId], foreignColumns: [t.id], name: 'project_types_parent_id_fkey' }).onDelete('set null'),
  ],
);
...
export const productProjectUses = pgTable(
  'product_project_uses',
  {
    id: bigserial('id', { mode: 'number' }).primaryKey(),
    productId: bigint('product_id', { mode: 'number' }).notNull().references(() => products.id, { onDelete: 'cascade' }),
    projectTypeId: bigint('project_type_id', { mode: 'number' }).notNull().references(() => projectTypes.id, { onDelete: 'cascade' }),
  ...
  (t) => [
    ...
    check('product_project_uses_confidence_check', sql`${t.confidence} is null or (${t.confidence} >= 0 and ${t.confidence} <= 1)`),
  ],
);
...
export const productRelations = pgTable(
  ...
  {
    id: bigserial('id', { mode: 'number' }).primaryKey(),
    sourceProductId: bigint('source_product_id', { mode: 'number' }).notNull().references(() => products.id, { onDelete: 'cascade' }),
    targetProductId: bigint('target_product_id', { mode: 'number' }).notNull().references(() => products.id, { onDelete: 'cascade' }),
  ...
  (t) => [
    uniqueIndex('product_relations_source_target_type_unique').on(t.sourceProductId, t.targetProductId, t.relationType),
    ...
    check('product_relations_distinct_products_check', sql`${t.sourceProductId} <> ${t.targetProductId}`),
    check('product_relations_confidence_check', sql`${t.confidence} is null or (${t.confidence} >= 0 and ${t.confidence} <= 1)`),
  ],
);
...
export const productRelationEvidence = pgTable(
  ...
  {
    id: bigserial('id', { mode: 'number' }).primaryKey(),
    relationId: bigint('relation_id', { mode: 'number' }).notNull().references(() => productRelations.id, { onDelete: 'cascade' }),
