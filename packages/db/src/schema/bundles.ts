...
import { bigint, bigserial, boolean, check, index, integer, numeric, pgEnum, pgTable, text, timestamp, uniqueIndex } from 'drizzle-orm/pg-core';

import { products } from './products';

export const bundlePricingModeEnum = pgEnum('bundle_pricing_mode', ['fixed', 'component_sum']);

export const bundleListings = pgTable('bundle_listings', {
...
}, (t) => [uniqueIndex('bundle_listings_product_unique').on(t.productId), index('idx_bundle_listings_active').on(t.active)]);

export const bundleComponents = pgTable('bundle_components', {
  id: bigserial('id', { mode: 'number' }).primaryKey(),
  bundleId: bigint('bundle_id', { mode: 'number' }).notNull().references(() => bundleListings.id, { onDelete: 'cascade' }),
...
export const aiPricingPolicies = pgTable('ai_pricing_policies', {
  id: integer('id').primaryKey().default(1),
  defaultMinimumGrossMargin: numeric('default_minimum_gross_margin', { precision: 5, scale: 4 }).notNull().default('0.1500'),
  allowRequestOverride: boolean('allow_request_override').notNull().default(true),
  updatedBy: text('updated_by'),
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
}, (t) => [check('ai_pricing_policies_margin_check', sql`${t.defaultMinimumGrossMargin} >= 0 and ${t.defaultMinimumGrossMargin} < 1`)]);
