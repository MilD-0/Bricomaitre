import { bigserial, bigint, text, numeric, timestamp, index } from 'drizzle-orm/pg-core';
...
export const processedOrders = adminSchema.table(
  'processed_orders',
  {
    ...
    productCost: numeric('product_cost', { precision: 12, scale: 2 }).notNull().default('0'),
    profit: numeric('profit', { precision: 12, scale: 2 }).notNull().default('0'),

    feeLivraison: numeric('fee_livraison', { precision: 10, scale: 2 }).notNull().default('0'),
    feePoids: numeric('fee_poids', { precision: 10, scale: 2 }).notNull().default('0'),
    feeExtra: numeric('fee_extra', { precision: 10, scale: 2 }).notNull().default('0'),
    feeSms: numeric('fee_sms', { precision: 10, scale: 2 }).notNull().default('0'),
    feeStockage: numeric('fee_stockage', { precision: 10, scale: 2 }).notNull().default('0'),
    feeCommission: numeric('fee_commission', { precision: 10, scale: 2 }).notNull().default('0'),
  ...
  (t) => [
    ...
    index('idx_po_stats_date').on(
      sql`coalesce(${t.encaissedAt}, ${t.deliveredAt}, ${t.orderCreatedAt})`,
    ),
    index('idx_po_wilaya_stats_date').on(
      t.wilaya,
      sql`coalesce(${t.encaissedAt}, ${t.deliveredAt}, ${t.orderCreatedAt})`,
    ),
    index('idx_po_delivery_stats_date').on(
      t.deliveryType,
      sql`coalesce(${t.encaissedAt}, ${t.deliveredAt}, ${t.orderCreatedAt})`,
    ),
  ],
