...
export function minimumSellingPriceForMargin(input: { purchaseCost: number; minimumGrossMargin?: number }) {
...
export function evaluateDiscountPrice(input: { proposedPrice: number; purchaseCost: number; minimumGrossMargin?: number }) {
  const minimumPrice = minimumSellingPriceForMargin(input);
  const grossMargin = calculateGrossMargin({ sellingPrice: input.proposedPrice, purchaseCost: input.purchaseCost });
