...
import { createRequestedProductContentChangesSchema, PRODUCT_CONTENT_GENERATION_INSTRUCTIONS, productContentChangesSchema, productContentGenerationInputSchema } from './product-content';
...
describe('product content AI contracts', () => {
  ...
  it('requires every requested field and rejects unrequested output fields', () => {
    ...
    expect(requested.safeParse({ titleAr: 'مثقاب', descriptionAr: 'وصف واضح.' }).success).toBe(true);
    expect(requested.safeParse({ titleAr: 'مثقاب' }).success).toBe(false);
    expect(requested.safeParse({ titleAr: 'مثقاب', descriptionAr: 'وصف واضح.', title: 'Drill' }).success).toBe(false);
  });

  it('keeps limitations out of customer-facing generated fields', () => {
    expect(PRODUCT_CONTENT_GENERATION_INSTRUCTIONS).toContain('existing French field as the factual source');
    expect(PRODUCT_CONTENT_GENERATION_INSTRUCTIONS).toContain('never meta commentary');
    expect(PRODUCT_CONTENT_GENERATION_INSTRUCTIONS).toContain('explain any limitation only in reasoning');
  });
});
