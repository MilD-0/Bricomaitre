...
export function ImageUploadField({
  ...
  const existingImages = useMemo(() => value.map((url, index) => ({ id: `${url}-${index}`, url, index })), [value]);
  ...
  const uploadImages = async (files: FileList | null) => {
    ...
    const effectiveFiles = pickerStateRef.current.multiple ? selectedFiles : selectedFiles.slice(0, 1);
    ...
    setUploads((current) => [
      ...current,
      ...nextUploads.map(({ file: _file, ...upload }) => upload),
    ]);
        ...
        nextUploads.map(async ({ id, file }) => {
          try {
            const urls = await uploadSingleImage({
              ...
              uploadUrl,
              onProgress: (progress) => {
                setUploads((current) => current.map((upload) => (upload.id === id ? { ...upload, progress } : upload)));
              },
            });
            setUploads((current) => current.map((upload) => (upload.id === id ? { ...upload, progress: 100, status: 'success' } : upload)));
            return urls;
          } catch {
            setUploads((current) => current.map((upload) => (upload.id === id ? { ...upload, status: 'error' } : upload)));
    ...
    <Field className="rounded-2xl border border-border/70 bg-background/60 p-3.5">
      ...
      <FieldContent className="gap-3">
        <input
          ref={inputRef}
          type="file"
          multiple={pickerStateRef.current.multiple}
          accept="image/*"
          className="sr-only"
              ...
              <div key={image.id} className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm">
                <div className="relative aspect-square bg-muted/30">
                  <button
                    ...
                    onClick={() => setPreviewImage({ src: image.url, alt: `Image ${image.index + 1}` })}
                  >
                    <img src={image.url} alt="" className="size-full object-cover" />
                  </button>
                </div>
                <div className="flex items-center justify-between gap-2 border-t border-border/60 px-2.5 py-2">
                  <span className="truncate text-[11px] font-medium text-muted-foreground">Uploaded</span>
              ...
              <div key={upload.id} className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm">
                <div className="relative aspect-square bg-muted/30">
                  <button
                    ...
                    className="block size-full cursor-pointer"
                    onClick={() => setPreviewImage({ src: upload.previewUrl, alt: upload.fileName })}
                  >
                    <img
                  ...
                  <div className="absolute inset-x-0 bottom-0 bg-linear-to-t from-[hsl(var(--background)/0.94)] via-[hsl(var(--background)/0.55)] to-transparent p-2">
                    <div className="h-1.5 overflow-hidden rounded-full bg-[hsl(var(--foreground)/0.12)]">
                      <div className="h-full rounded-full bg-primary transition-[width] duration-200" style={{ width: `${upload.progress}%` }} />
                    </div>
                  </div>
                  <div className="absolute inset-x-0 top-3 flex justify-center">
                    <div className="rounded-full border border-border/20 bg-[hsl(var(--background)/0.84)] px-2.5 py-1 text-[11px] font-medium text-foreground backdrop-blur-sm">
                      {upload.status === 'uploading' ? `${upload.progress}%` : upload.status === 'success' ? 'Done' : 'Failed'}
                    </div>
                  </div>
                ...
                <div className="space-y-1 border-t border-border/60 px-2.5 py-2">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    {upload.status === 'uploading' ? `${upload.progress}% uploaded` : upload.status === 'success' ? 'Uploaded' : 'Upload failed'}
                  </p>
                </div>
              </div>
            ))}
        ...
        <DialogContent className="max-w-5xl border-none bg-transparent p-0 shadow-none">
          {previewImage ? (
            <div className="overflow-hidden rounded-[1.75rem] border border-border/15 bg-[hsl(var(--background)/0.86)] p-3 shadow-[var(--shadow-vapor-strong)] backdrop-blur-xl">
              <img src={previewImage.src} alt={previewImage.alt} className="max-h-[85vh] w-full rounded-[1.25rem] object-contain" />
            </div>
          ) : null}
        </DialogContent>
      </Dialog>
