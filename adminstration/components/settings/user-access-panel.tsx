...
import { userAccessGrantFormSchema, type BuiltInRole, type UserAccessGrantFormValues } from '../../lib/permissions';
...
export function UserAccessPanel() {
  ...
  const invalidateAccess = () => queryClient.invalidateQueries({ queryKey: ['settings', 'access-grants'] });

  const createMutation = useMutation({
    mutationFn: (values: UserAccessGrantFormValues) => request('/api/settings/access', {
      method: 'POST',
      body: JSON.stringify(values),
    }),
    onMutate: () => ({ toastId: toast.loading(t('settings.accessManager.notifications.save.loading')) }),
    onSuccess: async (_data, _values, context) => {
      toast.success(t('settings.accessManager.notifications.save.success'), { id: context?.toastId });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, values }: { id: number; values: UserAccessGrantFormValues }) => request(`/api/settings/access/${id}`, {
      method: 'PUT',
      body: JSON.stringify(values),
    }),
    onMutate: () => ({ toastId: toast.loading(t('settings.accessManager.notifications.save.loading')) }),
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: () => ({ toastId: toast.loading(t('settings.accessManager.notifications.delete.loading')) }),
    onSuccess: async (_data, _id, context) => {
      toast.success(t('settings.accessManager.notifications.delete.success'), { id: context?.toastId });
      await invalidateAccess();
      await queryClient.invalidateQueries({ queryKey: ['action-history'] });
    ...
    <section className="overflow-hidden rounded-[1.75rem] border border-border/70 bg-card/90 shadow-sm">
      <div className="border-b border-border/70 bg-linear-to-r from-muted/45 via-background to-background px-5 py-4 sm:px-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold tracking-tight text-foreground">{t('settings.accessManager.title')}</h2>
          </div>
          <Badge className="rounded-full px-3 py-1">{t('settings.accessManager.count', { count: String(accessQuery.data.items.length) })}</Badge>
        </div>
      </div>
      ...
      <div className="grid gap-6 px-5 py-5 sm:px-6 sm:py-6 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        ...
        >
          <FieldGroup className="gap-4">
            <Field>
              <FieldLabel htmlFor="grant-email">{t('settings.accessManager.emailLabel')}</FieldLabel>
              <FieldContent>
                <Input id="grant-email" type="email" {...form.register('email')} placeholder={t('settings.accessManager.emailPlaceholder')} />
                {form.formState.errors.email ? <p className="text-xs text-rose-600">{form.formState.errors.email.message}</p> : null}
              </FieldContent>
            </Field>

            <Field>
              <FieldLabel htmlFor="grant-assignment">{t('settings.accessManager.assignmentLabel')}</FieldLabel>
              <FieldContent>
                <NativeSelect
                  id="grant-assignment"
                ...
                >
                  <NativeSelectOption value="">{t('settings.accessManager.assignmentPlaceholder')}</NativeSelectOption>
                ...
                {form.formState.errors.role ? <p className="text-xs text-rose-600">{form.formState.errors.role.message}</p> : null}
                {form.formState.errors.roleDefinitionId ? <p className="text-xs text-rose-600">{form.formState.errors.roleDefinitionId.message}</p> : null}
              </FieldContent>
            </Field>
          </FieldGroup>
          ...
          <div className="flex flex-wrap gap-2 border-t border-border/70 pt-2">
            <Button type="submit" disabled={isSubmitting}>
              {editingGrantId ? t('settings.accessManager.updateAction') : t('settings.accessManager.createAction')}
            </Button>
            {editingGrantId ? (
              <Button
                type="button"
        ...
        <div className="overflow-hidden rounded-[1.5rem] border border-border/70 bg-background/80">
          <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,0.8fr)_auto] gap-3 border-b border-border/70 bg-muted/25 px-4 py-3">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground whitespace-normal break-words">{t('settings.accessManager.columns.email')}</p>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground whitespace-normal break-words">{t('settings.accessManager.columns.assignment')}</p>
