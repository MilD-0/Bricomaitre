...
describe('UserAccessPanel', () => {
  ...
  it('creates and edits an access grant', async () => {
    ...
    server.use(
      http.get('/api/settings/access', () => HttpResponse.json({
        items,
      ...
      http.post('/api/settings/access', async ({ request }) => {
        const body = (await request.json()) as { email: string; role: string | null; roleDefinitionId: number | null };
        items.push({
          id: 2,
          email: body.email,
    ...
    expect(screen.getByText('settings.accessManager.columns.assignment')).toHaveClass('whitespace-normal');

    await userEvent.type(screen.getByPlaceholderText('settings.accessManager.emailPlaceholder'), 'employee@example.com');
    await userEvent.selectOptions(screen.getByLabelText('settings.accessManager.assignmentLabel'), 'built-in:employee');
    await userEvent.click(screen.getByRole('button', { name: 'settings.accessManager.createAction' }));
    ...
    await userEvent.selectOptions(screen.getByLabelText('settings.accessManager.assignmentLabel'), 'custom:7');
    await userEvent.click(screen.getByRole('button', { name: 'settings.accessManager.updateAction' }));
