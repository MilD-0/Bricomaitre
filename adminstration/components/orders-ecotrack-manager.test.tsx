...
describe('OrdersEcotrackManager', () => {
  ...
  it('renders the ECOTRACK shipments table with the requested columns', async () => {
    ...
    expect(screen.getByLabelText('ordersEcotrackManager.fields.scanTrackingNumber')).toBeInTheDocument();
  ...
  it('shows a partial-success toast and invalidates shipments after a mixed refresh result', async () => {
    ...
    await user.click(screen.getAllByRole('button', { name: 'ordersEcotrackManager.actions.refreshVisible' })[0]!);
  ...
  it('shows a critical partial-success toast for bulk dispatch failures and still invalidates queries', async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(async (input) => {
      ...
      if (url.includes('/api/orders/ecotrack/shipments/dispatch')) {
        return new Response(JSON.stringify({
          ok: true,
          items: [{ orderId: 11 }],
          failures: [{ orderId: 12, reference: '12', trackingNumber: 'TRK-12', message: 'Order #12 / Ref 12 / Tracking TRK-12: The shipment cannot be dispatched in its current Ecotrack state.' }],
          successCount: 1,
          failureCount: 1,
          totalRequested: 2,
        }), { status: 200, headers: { 'Content-Type': 'application/json' } });
      ...
      if (url.includes('/api/orders/ecotrack/shipments')) {
        return new Response(JSON.stringify(buildInitialOrders(2)), { status: 200, headers: { 'Content-Type': 'application/json' } });
      }
    ...
    await user.click(screen.getByRole('button', { name: 'ordersEcotrackManager.actions.dispatchReady' }));
    const dispatchButtons = await screen.findAllByRole('button', { name: 'ordersEcotrackManager.actions.dispatch' });
  ...
  it('downloads successful labels and shows a critical partial toast for failed labels', async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(async (input) => {
      ...
      if (url.includes('/api/orders/ecotrack/shipments/labels')) {
        return new Response(JSON.stringify({
          ...
          failures: [{ orderId: 12, reference: '12', trackingNumber: 'TRK-12', message: 'Order #12 / Ref 12 / Tracking TRK-12: The label is unavailable from Ecotrack.' }],
          successCount: 1,
          failureCount: 1,
          totalRequested: 2,
    ...
    await user.click(screen.getByRole('button', { name: 'ordersEcotrackManager.actions.printSelected' }));
  ...
  it('shows status-allowed row actions inside the row dropdown menu', async () => {
    ...
    const menuButtons = screen.getAllByRole('button', { name: 'ordersEcotrackManager.actions.dispatch menu' });
    ...
    expect(screen.getByRole('menuitem', { name: 'ordersEcotrackManager.actions.dispatch' })).toBeInTheDocument();
    expect(screen.getByRole('menuitem', { name: 'ordersEcotrackManager.actions.refresh' })).toBeInTheDocument();
    expect(screen.getByRole('menuitem', { name: 'ordersEcotrackManager.actions.label' })).toBeInTheDocument();
    expect(screen.getByRole('menuitem', { name: 'ordersEcotrackManager.actions.showHistory' })).toBeInTheDocument();
  });
  ...
  it('opens the finalize dialog when a dispatchable tracking number is scanned', async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      ...
      if (url.includes('/api/orders/ecotrack/shipments?')) {
        return new Response(JSON.stringify(buildDispatchableOrders(2)), { status: 200, headers: { 'Content-Type': 'application/json' } });
      }
    ...
    await user.type(screen.getByLabelText('ordersEcotrackManager.fields.scanTrackingNumber'), ' TRK-11 ');
    await user.click(screen.getByRole('button', { name: 'ordersEcotrackManager.actions.scanTrackingNumber' }));

    expect(await screen.findByText('ordersEcotrackManager.dialogs.finalizeTitle')).toBeInTheDocument();
    expect(screen.getByText('ordersEcotrackManager.dialogs.finalizeDescription:Ada Lovelace')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'ordersEcotrackManager.actions.saveOnly' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'ordersEcotrackManager.actions.saveAndDispatch' })).toBeInTheDocument();
  ...
  it('shows an error toast when a scanned tracking number is not found', async () => {
    ...
    await user.type(screen.getByLabelText('ordersEcotrackManager.fields.scanTrackingNumber'), 'TRK-999');
    await user.click(screen.getByRole('button', { name: 'ordersEcotrackManager.actions.scanTrackingNumber' }));
    ...
    expect(screen.queryByText('ordersEcotrackManager.dialogs.finalizeTitle')).not.toBeInTheDocument();
  });
  ...
  it('uses patch only when saving from finalize mode without dispatch', async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      ...
      if (url.includes('/api/orders/ecotrack/shipments/11') && init?.method === 'PATCH') {
        return new Response(JSON.stringify({ ok: true, item: buildShipment(11, 'Ada Lovelace', 'TRK-11', '0550123456', 'Chair', { canEdit: true, canDispatch: true }) }), { status: 200, headers: { 'Content-Type': 'application/json' } });
      }
    ...
    await user.type(screen.getByLabelText('ordersEcotrackManager.fields.scanTrackingNumber'), 'TRK-11');
    await user.click(screen.getByRole('button', { name: 'ordersEcotrackManager.actions.scanTrackingNumber' }));
    await user.click(await screen.findByRole('button', { name: 'ordersEcotrackManager.actions.saveOnly' }));

    await waitFor(() => {
      expect(toastMock.success).toHaveBeenCalledWith('ordersEcotrackManager.notifications.update.success', { id: 'toast-id' });
    });

    const calledUrls = fetchMock.mock.calls.map(([input]) => typeof input === 'string' ? input : input instanceof URL ? input.toString() : input.url);
    expect(calledUrls.some((url) => url.includes('/api/orders/ecotrack/shipments/dispatch'))).toBe(false);
  ...
  it('patches then dispatches when saving and dispatching from finalize mode', async () => {
    ...
    await user.type(screen.getByLabelText('ordersEcotrackManager.fields.scanTrackingNumber'), 'TRK-11');
    await user.click(screen.getByRole('button', { name: 'ordersEcotrackManager.actions.scanTrackingNumber' }));
    await user.click(await screen.findByRole('button', { name: 'ordersEcotrackManager.actions.saveAndDispatch' }));

    await waitFor(() => {
      expect(toastMock.success).toHaveBeenCalledWith('ordersEcotrackManager.notifications.finalize.success', { id: 'toast-id' });
    });

    const calledUrls = fetchMock.mock.calls.map(([input]) => typeof input === 'string' ? input : input instanceof URL ? input.toString() : input.url);
    expect(calledUrls.some((url) => url.includes('/api/orders/ecotrack/shipments/11'))).toBe(true);
    expect(calledUrls.some((url) => url.includes('/api/orders/ecotrack/shipments/dispatch'))).toBe(true);
  });
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      ...
      if (url.includes('/api/orders/ecotrack/shipments/dispatch')) {
        return new Response(JSON.stringify({
          ok: true,
          items: [],
          failures: [{ orderId: 11, reference: '11', trackingNumber: 'TRK-11', message: 'Dispatch blocked' }],
          successCount: 0,
          failureCount: 1,
          totalRequested: 1,
        }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  ...
  it('does not submit duplicate scan lookups while a scan request is pending', async () => {
    ...
    resolveLookup?.(new Response(JSON.stringify(buildDispatchableOrders(2)), { status: 200, headers: { 'Content-Type': 'application/json' } }));
