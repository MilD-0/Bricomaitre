...
const document = landingPageDocumentSchema.parse({
  ...
  blocks: [
    { id: 'hero', type: 'product-hero' as const, variant: 'media-left' as const, heading: 'Travaillez mieux', subheading: 'Une lampe solide.', imageUrl: null, imageAlt: '', primaryCtaLabel: 'Commander', showAddToCart: true },
...
describe('LandingDraftPreview', () => {
  ...
  it('localizes the permanent form preview for Arabic campaigns', () => {
    ...
    expect(screen.getByRole('region', { name: 'نموذج الطلب الدائم' })).toHaveAttribute('aria-label', 'نموذج الطلب الدائم');
  ...
  it('keeps the storefront preview on a light palette when the admin theme is dark', () => {
    render(<div className="dark"><LandingDraftPreview document={document} locale="fr" mobile={false} /></div>);
    const preview = screen.getByRole('region', { name: 'Formulaire de commande permanent' }).parentElement?.parentElement;
    expect(preview).toHaveClass('[color-scheme:light]', 'text-[#202529]');
    expect(screen.getByRole('region', { name: 'Formulaire de commande permanent' })).toHaveClass('text-[#202529]');
  });

  it('exposes every expanded composition and section treatment to administrators', () => {
    render(<LandingPageManager
      initialItems={[{ id: 1, productId: 7, productTitle: 'Lampe', locale: 'fr', slug: 'lampe', status: 'draft', draftRevision: 1, publishedRevision: null, updatedAt: '2026-07-18T00:00:00.000Z', document }]}
      products={[{ id: 7, title: 'Lampe', slug: 'lampe', brandId: null, categoryId: null, images: [] }]}
      storefrontBaseUrl="https://shop.example.com"
    />);

    for (const option of ['Introduction éditoriale', 'Galerie produit', 'Cas d’usage', 'Comparaison', 'Étapes', 'Bandeau de confiance', 'Offre produit dynamique']) {
  ...
  it('selects products with the shared searchable picker and derives the URL from the product slug', async () => {
    ...
    render(<LandingPageManager initialItems={[]} products={[{ id: 7, title: 'Lampe robuste', slug: 'lampe-robuste', brandId: null, categoryId: null, images: [] }]} storefrontBaseUrl="https://shop.example.com" />);
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/landing-pages', expect.objectContaining({
      method: 'POST',
  ...
  it('builds the public view link from the configured storefront environment URL', () => {
    render(<LandingPageManager
      initialItems={[{ id: 1, productId: 7, productTitle: 'Lampe', locale: 'fr', slug: 'lampe', status: 'published', draftRevision: 1, publishedRevision: 1, updatedAt: '2026-07-18T00:00:00.000Z', document }]}
      products={[]}
      storefrontBaseUrl="https://preview.example.com"
    />);
    expect(screen.getByRole('link', { name: 'Voir' })).toHaveAttribute('href', 'https://preview.example.com/fr/landing/lampe');
  });
});
