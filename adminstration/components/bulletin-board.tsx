...
function hasDraftContent(value: BulletinComposerFormValues) {
  return value.title.trim().length > 0 || value.body.trim().length > 0 || value.tagsInput.trim().length > 0 || value.attachments.length > 0;
}
...
export function BulletinBoard() {
  ...
  useEffect(() => {
    ...
    writeDraft(null);
  }, [composerOpen, draftValues.attachments, draftValues.body, draftValues.pinned, draftValues.tagsInput, draftValues.title, editingPost]);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async (_data, values, context) => {
      toast.success(t('notifications.create.success', { title: values.title }), { id: context?.toastId });
      await refreshBoard();
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onMutate: ({ values }) => ({ toastId: toast.loading(t('notifications.update.loading', { title: values.title })) }),
    onError: (_error, _values, context) => {
      toast.error(t('notifications.update.error'), { id: context?.toastId });
    },
    onSuccess: async (_data, { values }, context) => {
      toast.success(t('notifications.update.success', { title: values.title }), { id: context?.toastId });
      await refreshBoard();
      cancelComposer();
    },
  });
  ...
  const pinMutation = useMutation({
    ...
    onMutate: ({ pinned }) => ({
      toastId: toast.loading(t(pinned ? 'notifications.pin.loading' : 'notifications.unpin.loading')),
    }),
    onError: (_error, variables, context) => {
      toast.error(t(variables.pinned ? 'notifications.pin.error' : 'notifications.unpin.error'), { id: context?.toastId });
    },
    onSuccess: async (_data, variables, context) => {
      toast.success(t(variables.pinned ? 'notifications.pin.success' : 'notifications.unpin.success'), { id: context?.toastId });
      await refreshBoard();
    },
  });
  ...
  const replyDeleteMutation = useMutation({
    mutationFn: ({ replyId }: ReplyDeleteVariables) => request(`/api/bulletin/replies/${replyId}`, { method: 'DELETE' }),
    ...
    <div className="flex flex-col gap-4">
      <Card className="flex flex-col gap-4 rounded-[1.75rem] border-border/70 bg-background/95 p-5 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button type="button" variant="outline" size="sm" onClick={() => void refreshBoard()} disabled={boardQuery.isFetching}>
              <RefreshCcw data-icon="inline-start" className={boardQuery.isFetching ? 'animate-spin' : ''} />
              {t('actions.refresh')}
            </Button>
            <Button type="button" onClick={composerOpen && !editingPost ? cancelComposer : openComposer} disabled={!boardQuery.data.permissions.canPost}>
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <NativeSelect aria-label={t('filters.sortLabel')} value={sort} onChange={(event) => setSort(event.target.value as typeof sort)}>
            <NativeSelectOption value="updated-desc">{t('filters.sortUpdatedDesc')}</NativeSelectOption>
            <NativeSelectOption value="updated-asc">{t('filters.sortUpdatedAsc')}</NativeSelectOption>
            <NativeSelectOption value="created-desc">{t('filters.sortCreatedDesc')}</NativeSelectOption>
          </NativeSelect>
        </div>
      </Card>
        ...
        <Card className="flex flex-col gap-4 rounded-[1.75rem] border-border/70 bg-background/95 p-5 shadow-sm">
          ...
          <form className="flex flex-col gap-4" onSubmit={onSubmit}>
            <Field data-invalid={Boolean(form.formState.errors.title)}>
              ...
              <Input id="bulletin-title" aria-invalid={Boolean(form.formState.errors.title)} placeholder={t('composer.titlePlaceholder')} {...form.register('title')} />
            ...
            <Field data-invalid={Boolean(form.formState.errors.body)}>
              ...
              <Textarea id="bulletin-body" aria-invalid={Boolean(form.formState.errors.body)} placeholder={t('composer.bodyPlaceholder')} {...form.register('body')} />
            ...
            <Field data-invalid={Boolean(form.formState.errors.tagsInput)}>
              ...
              <Input id="bulletin-tags" aria-invalid={Boolean(form.formState.errors.tagsInput)} placeholder={t('composer.tagsPlaceholder')} {...form.register('tagsInput')} />
            ...
            <FileUploadField
              ...
              onChange={(files) => form.setValue('attachments', files, { shouldDirty: true, shouldValidate: true })}
            />
            {form.formState.errors.attachments ? <p className="text-xs text-destructive">{form.formState.errors.attachments.message}</p> : null}

            <Field orientation="horizontal" className="rounded-xl border border-border/70 bg-muted/30 px-3 py-2 text-sm text-foreground">
              <Checkbox
                id="bulletin-pinned"
                {...form.register('pinned')}
                disabled={!boardQuery.data.permissions.canModerate && !editingPost?.permissions.canPin}
              />
              <FieldContent>
            ...
            <div className="flex flex-wrap gap-2">
              <Button type="submit" disabled={busy || !boardQuery.data.permissions.canPost}>
                {createMutation.isPending || updateMutation.isPending ? <LoaderCircle data-icon="inline-start" className="animate-spin" /> : <SquarePen data-icon="inline-start" />}
      ...
      {boardQuery.isError ? (
        <Card className="rounded-[1.5rem] border-destructive/30 bg-destructive/5 p-6">
          ...
          <Button type="button" variant="outline" size="sm" className="mt-4" onClick={() => void refreshBoard()}>
            {t('actions.refresh')}
          </Button>
        </Card>
      ) : null}
      ...
      <div className="flex flex-col gap-4">
        {pinnedPosts.length > 0 ? (
          <section className="flex flex-col gap-3">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-semibold uppercase tracking-[0.16em] text-muted-foreground">{t('sections.pinned')}</p>
            </div>
        ...
        <section className="flex flex-col gap-3">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm font-semibold uppercase tracking-[0.16em] text-muted-foreground">{t('sections.recent')}</p>
            <p className="text-xs text-muted-foreground">{t('sections.visibleCount', { count: sortedPosts.length })}</p>
          </div>
        ...
        {sortedPosts.length > pageSize ? (
          <Card className="flex flex-col gap-3 rounded-[1.5rem] border-border/70 bg-background/95 p-4 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-muted-foreground">{t('pagination.page', { page: currentPage, total: totalPages })}</p>
            <div className="flex items-center gap-2">
      ...
      <Dialog open={deletePost !== null} onOpenChange={(open) => { if (!open) setDeletePost(null); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('delete.title')}</DialogTitle>
            <DialogDescription>{t('delete.description', { title: deletePost?.title ?? '' })}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setDeletePost(null)}>
              {t('actions.cancel')}
            </Button>
            <Button type="button" variant="destructive" disabled={deleteMutation.isPending} onClick={() => deletePost && deleteMutation.mutate(deletePost.id)}>
              {deleteMutation.isPending ? <LoaderCircle data-icon="inline-start" className="animate-spin" /> : null}
              {t('actions.delete')}
            </Button>
          </DialogFooter>
        </DialogContent>
...
function ReactionRow({
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{addReactionLabel}</span>
        {options.map((emoji) => {
        ...
        <div className="flex flex-col gap-1">
          {reactions.map((reaction) => (
            <p key={reaction.emoji} className="text-xs text-muted-foreground">
              ...
              {reaction.users
                .map((user) => (currentUserId !== null && user.id === currentUserId ? byYouLabel : user.name))
                .join(', ')}
            </p>
          ))}
        </div>
...
function BulletinPostCard({
  ...
  return (
    <Card className={`flex flex-col gap-4 rounded-[1.5rem] border-border/70 p-5 ${post.pinned ? 'bg-muted/30' : 'bg-background/95'}`}>
            ...
            >
              <div className="flex items-center gap-3">
                {attachment.contentType.startsWith('image/') ? (
                  <img src={attachment.fileUrl} alt={attachment.fileName} className="size-14 rounded-lg object-cover" />
                ) : (
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-border/70 bg-muted/20 p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <Button type="button" variant="outline" size="sm" disabled={busy} onClick={() => setReplyOpen((value) => !value)}>
            {labels.refreshReply}
          </Button>
        </div>
        ...
        {replyOpen ? (
          <div className="flex flex-col gap-3 rounded-xl border border-border/70 bg-background/80 p-3">
            <Textarea value={replyBody} onChange={(event) => setReplyBody(event.target.value)} placeholder={labels.replyPlaceholder} />
            <div className="flex flex-wrap gap-2">
              <Button
                type="button"
                size="sm"
        ...
        {post.replies.length > 0 ? (
          <div className="grid gap-3">
            {post.replies.map((reply) => (
              <div key={reply.id} className="rounded-xl border border-border/70 bg-background/80 p-3">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    <p className="text-sm font-medium text-foreground">{currentUserId !== null && reply.author.id === currentUserId ? labels.byYou : reply.author.name}</p>
                    <p className="text-xs text-muted-foreground">{new Date(reply.updatedAt).toLocaleString()}</p>
                  </div>
                  {reply.permissions.canDelete ? (
                    <Button type="button" variant="ghost" size="sm" disabled={busy} onClick={() => setDeleteReply(reply)}>
                      <Trash2 data-icon="inline-start" />
                      {labels.deleteReply}
                    </Button>
                  ) : null}
      ...
      <Dialog open={deleteReply !== null} onOpenChange={(open) => { if (!open) setDeleteReply(null); }}>
        <DialogContent>
          <DialogHeader>
