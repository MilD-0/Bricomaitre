...
import { Download, FileText, ImagePlus, LoaderCircle, Paperclip, Trash2, XCircle } from 'lucide-react';
...
function isPreviewableImage(file: { contentType?: string | null; fileName?: string | null }) {
  return file.contentType?.startsWith('image/') || /\.(png|jpe?g|gif|webp|svg)$/i.test(file.fileName ?? '');
}
...
export function FileUploadField({
  ...
  const existingFiles = useMemo(() => value.map((file, index) => ({ ...file, index, id: `${file.fileUrl}-${index}` })), [value]);
  ...
  useEffect(() => {
    ...
    uppy.on('file-added', (file) => {
      setUploads((current) => [
        ...current,
        {
          id: file.id,
          fileName: file.name,
          previewUrl: file.type?.startsWith('image/') ? URL.createObjectURL(file.data as File) : null,
          progress: 0,
          status: 'uploading',
        },
      ]);
    ...
    uppy.on('upload-progress', (file, progress) => {
      ...
      const ratio = progress.bytesTotal ? Math.round((progress.bytesUploaded / progress.bytesTotal) * 100) : 0;
      setUploads((current) => current.map((upload) => (upload.id === file.id ? { ...upload, progress: ratio } : upload)));
    });
        ...
        {existingFiles.length > 0 || uploads.length > 0 ? (
          <div className="grid gap-3 sm:grid-cols-2">
            {existingFiles.map((file) => (
              <div key={file.id} className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm">
                <div className="flex items-start gap-3 p-3">
                  {isPreviewableImage(file) ? (
                    ...
                    >
                      <img src={file.fileUrl} alt={file.fileName} className="size-16 rounded-xl object-cover" />
                    </button>
                  ) : (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <a href={file.fileUrl} target="_blank" rel="noreferrer" className="inline-flex">
                        <Button type="button" variant="outline" size="sm">
                          <Download data-icon="inline-start" />
                          Download
                        </Button>
                      </a>
                      <Button type="button" variant="destructive" size="sm" onClick={() => setDeleteIndex(file.index)}>
            ...
            {uploads.map((upload) => (
              <div key={upload.id} className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm">
                <div className="flex items-start gap-3 p-3">
                  {upload.previewUrl ? (
                    <img src={upload.previewUrl} alt={upload.fileName} className="size-16 rounded-xl object-cover" />
                  ) : (
                    <div className="flex size-16 shrink-0 items-center justify-center rounded-xl border border-border/70 bg-muted/30 text-muted-foreground">
                      <Paperclip />
                    </div>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
                      {upload.status === 'uploading' ? <LoaderCircle className="size-3.5 animate-spin" /> : null}
                      {upload.status === 'error' ? <XCircle className="size-3.5 text-destructive" /> : null}
                      <span>{upload.status === 'error' ? 'Upload failed' : `${upload.progress}% uploaded`}</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <DialogContent className="max-w-5xl border-none bg-transparent p-0 shadow-none">
          {previewFile ? (
            <div className="overflow-hidden rounded-[1.75rem] border border-border/15 bg-[hsl(var(--background)/0.86)] p-3 shadow-[var(--shadow-vapor-strong)] backdrop-blur-xl">
              <img src={previewFile.src} alt={previewFile.alt} className="max-h-[85vh] w-full rounded-[1.25rem] object-contain" />
            </div>
          ) : null}
        </DialogContent>
      </Dialog>
