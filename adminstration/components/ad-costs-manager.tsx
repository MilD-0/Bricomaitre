...
export function AdCostsManager({
  ...
  const deleteCampaignMutation = useMutation({
    mutationFn: async (ids: string[]) => {
      await Promise.all(ids.map((id) => request(`/api/stats/ad-costs?id=${id}`, { method: 'DELETE' })));
    },
    onSuccess: async () => {
    ...
    <div className="flex flex-col gap-4">
      <div className="rounded-[1.6rem] border border-border/70 bg-linear-to-br from-card via-card to-muted/20 p-4">
        <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">{t('managerTitle')}</p>
            <p className="mt-1 text-sm text-muted-foreground">{t('fields.importHint')}</p>
          </div>
          <Button type="button" variant={showForm ? 'default' : 'outline'} onClick={() => setShowForm((value) => !value)}>
            {t('actions.add')}
          </Button>
        </div>
      ...
      {showForm ? (
        <div className="rounded-[1.6rem] border border-border/70 bg-muted/20 p-4">
          <p className="mb-3 text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">{t('actions.add')}</p>
          <div className="grid gap-3 md:grid-cols-3">
            <Input className="bg-background" type="date" value={form.date} onChange={(event) => setForm((current) => ({ ...current, date: event.target.value }))} />
            <Input className="bg-background" value={form.platform} onChange={(event) => setForm((current) => ({ ...current, platform: event.target.value }))} placeholder={t('fields.platform')} />
            <Input className="bg-background" value={form.campaignName} onChange={(event) => setForm((current) => ({ ...current, campaignName: event.target.value }))} placeholder={t('fields.campaignName')} />
            <Input className="bg-background" type="number" value={form.spend} onChange={(event) => setForm((current) => ({ ...current, spend: event.target.value }))} placeholder={t('fields.spend')} />
            <Input className="bg-background" type="number" value={form.impressions} onChange={(event) => setForm((current) => ({ ...current, impressions: event.target.value }))} placeholder={t('fields.impressions')} />
            <Input className="bg-background" type="number" value={form.clicks} onChange={(event) => setForm((current) => ({ ...current, clicks: event.target.value }))} placeholder={t('fields.clicks')} />
            <Input className="bg-background" type="number" value={form.conversions} onChange={(event) => setForm((current) => ({ ...current, conversions: event.target.value }))} placeholder={t('fields.conversions')} />
          </div>
          <div className="mt-4">
            <Button type="button" onClick={() => addMutation.mutate()} disabled={addMutation.isPending}>{t('actions.save')}</Button>
          </div>
        </div>
      ) : null}
      ...
      <div className="grid gap-4">
        {grouped.map((group) => (
          <div key={`${group.platform}-${group.campaignName}`} className="rounded-[1.35rem] border border-border/70 bg-muted/20 p-3.5">
            <div className="flex items-center justify-between gap-3">
              <button type="button" className="flex items-center gap-3 text-left" onClick={() => toggleCampaign(group.key)}>
                {group.entries.length > 1 ? (
                  expandedCampaigns.includes(group.key) ? <ChevronDown className="size-4 text-muted-foreground" /> : <ChevronRight className="size-4 text-muted-foreground" />
                ) : (
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {group.startDate === group.endDate ? formatDate(group.startDate) : `${formatDate(group.startDate)} - ${formatDate(group.endDate)}`}
                  </p>
                </div>
              </button>
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  className="size-9"
                  variant="ghost"
                  onClick={() => deleteCampaignMutation.mutate(group.entries.map((entry) => entry.id))}
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <div className="rounded-xl border border-border/70 bg-background/70 px-3 py-2">
                ...
                <p className="mt-1 text-sm font-semibold">
                  {group.totalConversions > 0 ? `${(group.totalSpend / group.totalConversions).toFixed(0)}` : '—'}
                </p>
              </div>
            </div>
                  ...
                  <TableBody>
                    {group.entries.map((entry) => (
                      <TableRow key={entry.id}>
                        ...
                        <TableCell>{entry.clicks ? (entry.spend / entry.clicks).toFixed(0) : '—'}</TableCell>
                        <TableCell>{entry.conversions ? (entry.spend / entry.conversions).toFixed(0) : '—'}</TableCell>
                        <TableCell>
                          <Button type="button" size="sm" variant="outline" onClick={() => deleteMutation.mutate(entry.id)}>{t('actions.delete')}</Button>
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
