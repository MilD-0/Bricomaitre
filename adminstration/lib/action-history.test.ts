...
describe('action-history helpers', () => {
  ...
  it('undoes and redoes an update action log', async () => {
    const historyEntry = {
      ...
      entityLabel: '0550',
      operation: 'update',
      beforeState: { id: 11, phoneNumber1: '0550', confirmed: 0, createdAt: '2026-03-20T00:00:00.000Z', updatedAt: '2026-03-20T00:00:00.000Z' },
    ...
    const tx = {
      select: vi.fn()
        ...
        .mockReturnValueOnce(createHistoryListSelectBuilder([
          { id: 6, isUndone: false },
          { id: 7, isUndone: false },
        ]))
  ...
  it('ignores removed columns when replaying historical snapshots', async () => {
    ...
    expect(entityUpdateSet).toHaveBeenCalledWith(expect.objectContaining({
      inventoryQuantity: 2,
      title: 'Widget',
    }));
    expect(entityUpdateSet).not.toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('extracts field-level changes for update entries', () => {
    expect(getActionHistoryChanges({
      operation: 'update',
      beforeState: {
        id: 9,
        inStock: false,
