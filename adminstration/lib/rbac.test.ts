...
describe('rbac helpers', () => {
  ...
  it('returns 403 when the user lacks permission for the resource', async () => {
    authMock.mockResolvedValue({ user: { isAllowed: true, role: 'employee', permissions: ['products_write'] } });
  ...
  it('returns null when the user is authorized', async () => {
    authMock.mockResolvedValue({ user: { isAllowed: true, role: 'admin', permissions: ['assets_write'] } });
