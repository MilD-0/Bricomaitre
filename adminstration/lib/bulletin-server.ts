...
async function getOrCreateTagIds(tx: BulletinTransaction, names: string[]) {
  ...
  if (missingNames.length > 0) {
    ...
    if (inserted.length !== missingNames.length) {
      const reloaded = await tx.select().from(bulletinTags).where(inArray(bulletinTags.name, names));
...
export async function syncBulletinPostTags(tx: BulletinTransaction, postId: number, tagNames: string[]) {
...
export function mapBulletinPosts(
  ...
  replyRows.forEach((row) => {
    const reactions = Array.from(replyReactionsByReplyId.get(row.id)?.values() ?? []).sort((left, right) => right.count - left.count || left.emoji.localeCompare(right.emoji));
...
export async function loadBulletinTagNames() {
  const tags = await getDb().select({ name: bulletinTags.name }).from(bulletinTags).orderBy(asc(bulletinTags.name));
