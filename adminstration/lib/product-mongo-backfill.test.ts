...
import { buildMongoProductTitleIndex, normalizeProductTitle, planProductMongoBackfill } from './product-mongo-backfill';

describe('lib/product-mongo-backfill', () => {
  ...
  it('separates duplicate mongo titles from unique entries', () => {
    ...
    expect(result.uniqueByTitle.get('office chair')).toEqual({ _id: 'mongo-3', title: 'Office Chair' });
