...
export function formatAiTaskTerminalMessage(input: {
  ...
  if (input.progress?.total !== undefined) {
    lines.push(`Progress: ${input.progress.current ?? 0}/${input.progress.total}${input.progress.phase ? ` · ${input.progress.phase}` : ''}`);
  }
  const summary = input.summary ?? {};
  const summaryParts = summaryKeys.flatMap((key) => key in summary ? [`${key}: ${String(summary[key])}`] : []);
  ...
  if (typeof summary.proposed === 'number' && summary.proposed > 0) {
    lines.push(`${summary.proposed} proposal${summary.proposed === 1 ? ' is' : 's are'} awaiting review and ${summary.proposed === 1 ? 'has' : 'have'} not been applied.`);
  }
  if (input.status === 'completed' && summary.complete !== true && 'complete' in summary) {
    lines.push('The task is not being reported as fully reconciled because the server did not confirm complete: true.');
  }
  return lines.join('\n');
}
...
export async function publishAiTaskTerminalMessage(input: {
  ...
  const db = getDb();
  const [existing] = await db.select({ id: aiMessages.id })
    .from(aiMessages)
    .where(and(
