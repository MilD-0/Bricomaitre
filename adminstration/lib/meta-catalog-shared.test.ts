...
describe('buildMetaCatalogExportRows', () => {
  ...
  it('uses the configured storefront base url when building product links', () => {
    process.env.STOREFRONT_BASE_URL = 'https://staging.example.com/';
    ...
    expect(rows[0]?.link).toBe('https://staging.example.com/products/configured-domain');
  ...
  it('serializes rows to csv with the shared header order', () => {
    ...
    expect(csv).toContain('id,content_id,title,description,availability,condition,price,sale_price,link,image_link,brand');
