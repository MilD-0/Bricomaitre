...
const conversation = { id: 7, sessionKey: '08ae5e04-bc1e-4b38-870f-49000e23663a', title: 'Catalog help' };
...
describe('admin AI chat response stream', () => {
  it('delivers incremental text before the final tool and conversation data', async () => {
    ...
    const response = new Response(new ReadableStream({
      start(controller) {
        ...
        controller.enqueue(encoder.encode(`${JSON.stringify({ type: 'text-delta', delta: 'answer.' })}\n${JSON.stringify({ type: 'result', toolResults: [], conversation })}\n`));
        controller.close();
      },
    ...
    await consumeAdminAiChatResponse(response, { onTextDelta: (delta) => deltas.push(delta), onResult });
  ...
  it('accepts the previous JSON payload during rollout', async () => {
    ...
    const onResult = vi.fn();
    await consumeAdminAiChatResponse(Response.json({ message: 'Done', toolResults: [], conversation }), { onTextDelta, onResult });
