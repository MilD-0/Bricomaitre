...
export const CANCELLABLE_ADMIN_BACKGROUND_JOB_TYPES = [
  'ai_categorization',
  'ai_content',
  'product_export',
  'catalog_feed_refresh',
...
const typeByQueue = new Map(Object.entries(queueByType).map(([type, queue]) => [queue, type as AdminBackgroundJobType]));
  ...
  switch (input.type) {
    ...
    case 'order_export': {
      ...
      if (!input.orderMode || orderIds.length === 0) {
        return { error: 'orderMode and at least one resolved order ID are required for an order export.' };
      }
      return startOrderExportJob(input.actor.email, { mode: input.orderMode, orderIds }, undefined, taskContext);
    }
    ...
    case 'ecotrack_shipment_sync':
      return startEcotrackShipmentSyncJob(input.actor.email, trigger, input.actor, undefined, taskContext);
  }
}
