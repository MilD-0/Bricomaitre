...
const landingPagePlanSchema = z.object({
  archetype: z.enum(['visual-showcase', 'problem-solution', 'technical-guide', 'use-case-journey', 'direct-response']),
  theme: z.object({
  ...
  hero: z.object({
    variant: z.enum(['media-left', 'media-right', 'media-background', 'product-stage', 'editorial']),
  ...
  for (const [index, section] of plan.sections.entries()) {
    if (seen.has(section.type)) context.addIssue({ code: 'custom', path: ['sections', index, 'type'], message: 'Planned section types must be unique.' });
...
export interface LandingPageStageRunner {
  generatePlan(input: LandingPageGenerationInput): Promise<{ plan: LandingPagePlan; usage: TokenUsage }>;
  generateBlock(input: { generationInput: LandingPageGenerationInput; section: PlannedSection; index: number }): Promise<{ block: LandingPageBlock; usage: TokenUsage }>;
}
...
export async function generateLandingPageDraft(input: {
  ...
  try {
    ...
    return {
      ...
      reasoning: 'The structured content model was unavailable or returned an invalid plan, so a safe catalog-grounded fallback was created for review.',
      groundingNotes: ['Fallback content uses the verified catalog title, description, and first product image only.'],
      usage: {},
      model: 'deterministic-v1',
      stages: { status: 'full-fallback', plannedSections: 0, generatedSections: 0, fallbackSections: input.fallbackDocument.blocks.length, skippedSections: 0 },
    };
  }
}
...
function blockSchemaFor(type: PlannedSection['type']): z.ZodTypeAny {
  switch (type) {
    case 'benefit-grid': return landingPageBenefitsBlockSchema;
    case 'media-feature': return landingPageMediaFeatureBlockSchema;
    case 'specifications': return landingPageSpecificationsBlockSchema;
    case 'faq': return landingPageFaqBlockSchema;
    case 'editorial-intro': return landingPageEditorialIntroBlockSchema;
    case 'image-gallery': return landingPageImageGalleryBlockSchema;
    case 'use-cases': return landingPageUseCasesBlockSchema;
    case 'process': return landingPageProcessBlockSchema;
    case 'trust-band': return landingPageTrustBandBlockSchema;
    case 'commerce-panel': return landingPageCommercePanelBlockSchema;
  }
}
  ...
  return {
    async generatePlan(input) {
      const result = await generateText({
        ...
        output: Output.object({ schema: landingPagePlanSchema, name: 'storefront_landing_page_plan' }),
    ...
    async generateBlock({ generationInput, section, index }) {
      ...
      const result = await generateText({
        ...
        prompt: JSON.stringify({ product: generationInput.product, locale: generationInput.locale, campaignAngle: generationInput.campaignAngle, plannedSection: section }),
        output: Output.object({ schema, name: `storefront_landing_page_${section.type.replaceAll('-', '_')}` }),
        maxRetries: config.maxRetries,
        timeout: config.requestTimeoutMs,
      });
      const rawBlock = schema.parse(await result.output) as Record<string, unknown>;
      const block = landingPageBlockSchema.parse({ ...rawBlock, id: `section-${index + 1}-${section.type}`, type: section.type, surface: section.surface, width: section.width });
...
async function generateSections(
  ...
  for (let offset = 0; offset < sections.length; offset += 2) {
    ...
    results.push(...await Promise.allSettled(batch.map((section, index) => runner.generateBlock({ generationInput, section, index: offset + index }))));
  }
  return results;
}

function localFallbackFor(input: LandingPageGenerationInput) {
  const title = input.locale === 'ar' && input.product.titleAr ? input.product.titleAr : input.product.title;
  const description = input.locale === 'ar' ? input.product.descriptionAr : input.product.description;
  return buildDefaultLandingPageDocument({ locale: input.locale, title, description, imageUrl: input.product.images[0] ?? null });
}

export function createLandingPageGenerator(config: AiConfig = getAiConfig(), stageRunner?: LandingPageStageRunner): LandingPageGenerator {
  ...
  return {
    async generate(rawInput) {
      ...
      const eligibleSections = plan.sections.filter((section) => section.type !== 'image-gallery' || new Set(input.product.images).size >= 2);
      ...
      for (const block of fallback.blocks) {
        ...
        if (block.type === 'product-hero' || block.type === 'final-cta' || presentTypes.has(block.type)) continue;
      ...
      const document = landingPageDocumentSchema.parse({
        ...
        blocks: [
          { id: 'hero', type: 'product-hero', ...plan.hero, surface: 'plain', width: 'wide', imageUrl, imageAlt: title, showAddToCart: true },
          ...generatedBlocks,
          ...fallbackBlocks,
          { id: 'final', type: 'final-cta', ...plan.finalCta, surface: 'accent', width: 'wide', imageUrl, imageAlt: title },
        ],
      });
      const skippedSections = plan.sections.length - generatedBlocks.length;
      const status = skippedSections > 0 || fallbackBlocks.length > 0 ? 'partial-fallback' : 'completed';
      ...
      return {
        ...
        groundingNotes: [...plan.groundingNotes, `Staged generation: ${generatedBlocks.length} generated section(s), ${fallbackBlocks.length} deterministic section fallback(s).`],
        usage,
        model: resolveAiModel(config, 'content'),
        stages: { status, plannedSections: plan.sections.length, generatedSections: generatedBlocks.length, fallbackSections: fallbackBlocks.length, skippedSections },
      };
    },
  };
}
