...
function createDbMock(rows: Array<ReturnType<typeof createShipmentRow>>, options?: { limitSequence?: number[] }) {
...
describe('admin ecotrack shipment reconciliation', () => {
  ...
  it('records chunk-level fetch failures for every row in that chunk', async () => {
    ...
    expect(result.failures).toEqual([
      { orderId: 11, reference: '11', trackingNumber: 'TRK-11', message: 'Order #11 / Ref 11 / Tracking TRK-11: Status fetch failed' },
      { orderId: 12, reference: '12', trackingNumber: 'TRK-12', message: 'Order #12 / Ref 12 / Tracking TRK-12: Status fetch failed' },
    ]);
  });
});
