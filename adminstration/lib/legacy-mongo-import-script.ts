...
export function resolveLegacyImportFiles(options: LegacyImportCliOptions): LegacyImportFilePaths {
  ...
  const filePaths = {
    brands: options.brandsPath ? resolve(options.brandsPath) : dir ? resolve(dir, 'mongo-brands.json') : null,
    categories: options.categoriesPath ? resolve(options.categoriesPath) : dir ? resolve(dir, 'mongo-categories.json') : null,
    products: options.productsPath ? resolve(options.productsPath) : dir ? resolve(dir, 'mongo-products.json') : null,
    orders: options.ordersPath ? resolve(options.ordersPath) : dir ? resolve(dir, 'mongo-orders.json') : null,
...
export function parseDropTables(rawValue: string | null) {
  ...
  const invalid = values.filter((value): value is string => !IMPORT_TARGETS.includes(value as ImportTarget));
  if (invalid.length > 0) {
