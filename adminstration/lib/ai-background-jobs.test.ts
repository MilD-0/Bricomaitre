...
describe('admin AI background job control', () => {
  ...
  it('lists recent snapshots across every registered admin queue', async () => {
    ...
    await expect(listAdminBackgroundJobs(25)).resolves.toEqual([expect.objectContaining({
      id: 'job-1',
      type: 'ai_categorization',
      cancellable: true,
    })]);

    expect(mocks.list).toHaveBeenCalledWith(expect.arrayContaining([
  ...
  it('retrieves and cancels an exact cooperatively cancellable job', async () => {
    ...
    await expect(getAdminBackgroundJob('ai_categorization', 'job-1')).resolves.toMatchObject({ id: 'job-1' });
  ...
  it('refuses to claim cancellation support for non-cooperative jobs', async () => {
    ...
    expect(result).toMatchObject({ error: expect.stringContaining('does not support cooperative cancellation') });
