...
export function getDefaultAuthorizedHref({
  ...
  const orderedKeys: NavigationKey[] = ['administration', 'products', 'orders', 'inventory', 'assets', 'brandsCategories', 'stats', 'bulletin'];
  ...
  const firstAccessibleKey = orderedKeys.find((key) => canAccessNavigationItem({ isAllowed, key, permissions, role }));
