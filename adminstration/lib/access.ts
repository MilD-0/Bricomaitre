...
import { roleDefinitionPermissions, roleDefinitions, userAccessGrants, users } from '../db/schema';
import {
...
export async function buildAccessProfile(seed: AccessSeed): Promise<AccessProfile> {
  ...
  if (typeof accessGrant.roleDefinitionId === 'number' && Number.isFinite(accessGrant.roleDefinitionId)) {
  ...
  return {
    ...
    role: resolvedRole,
    roleDefinitionId: null,
    roleLabel: typeof seed.roleLabel === 'string' && seed.roleLabel.trim().length > 0 ? seed.roleLabel.trim() : null,
  };
}
