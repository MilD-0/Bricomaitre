...
export const adminAiConversationSchema = z.object({
...
export const adminAiChatStreamEventSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('result'), toolResults: z.unknown(), conversation: adminAiConversationSchema }).strict(),
...
export async function consumeAdminAiChatResponse(
  ...
  if (!contentType.includes('application/x-ndjson')) {
    const body = await response.json() as { message?: unknown; error?: unknown; toolResults?: unknown; conversation?: unknown };
    if (!response.ok) throw new Error(typeof body.error === 'string' ? body.error : 'admin_ai_failed');
    if (typeof body.message === 'string' && body.message) handlers.onTextDelta(body.message);
    handlers.onResult({ toolResults: body.toolResults, conversation: adminAiConversationSchema.parse(body.conversation) });
    return;
  }
