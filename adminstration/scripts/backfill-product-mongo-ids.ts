...
import { planProductMongoBackfill, type MongoProductSource } from '../lib/product-mongo-backfill';
...
function parseArgs(argv: string[]): ScriptOptions {
  ...
  if (!filePath) {
    throw new Error('Usage: tsx scripts/backfill-product-mongo-ids.ts <mongo-products.json> [--dry-run] [--overwrite]');
  }
...
async function main() {
  ...
  const db = getDb();
  const postgresProducts = await db.select({
    id: products.id,
  ...
  if (plan.duplicateMongoTitles.length > 0) {
    ...
    for (const duplicate of plan.duplicateMongoTitles.slice(0, 20)) {
      console.log(`- ${duplicate.normalizedTitle}: ${duplicate.items.map((item) => item._id).join(', ')}`);
    }
  }
