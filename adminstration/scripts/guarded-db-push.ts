...
async function main() {
  ...
  if (!allowNonLocal && !isLocalHostname(hostname)) {
    throw new Error(
      `Refusing to run db:push against non-local host "${hostname}". Use pnpm --filter adminstration db:verify or pnpm --filter adminstration db:migrate instead, or set ALLOW_NONLOCAL_DB_PUSH=true to override.`,
    );
  }
