...
describe('scripts/import-legacy-mongo', () => {
  it('aborts by default when blocked orders are present', () => {
    expect(shouldAbortForBlockedOrders(
      { skipBlockedOrders: false },
      { orderDiagnostics: { blockedByCart: [{ mongoId: 'order-1', missingRefs: ['missing-product'] }] } },
    )).toBe(true);
  });
