...
} from '../lib/legacy-mongo-import-script';
import {
  importMongoBrands,
...
async function main() {
  ...
  if (shouldAbortForBlockedOrders(options, prepared.report)) {
    throw new Error('Refusing to import because some orders reference products that are not available for remapping.');
  }
  ...
  await db.transaction(async (tx) => {
    ...
    const brandIdByMongoId = selectedTargets.includes('brands')
      ? await importMongoBrands(prepared.brands.map((brand) => ({
        _id: brand.mongoId,
        name: brand.name,
...
function prepareImportData(
  ...
  report.counts.categories.prepared = preparedCategories.filter((category) => typeof category.name === 'string' && category.name.trim().length > 0).length;
  report.counts.categories.skipped = report.counts.categories.loaded - report.counts.categories.prepared;

  const availableProductMongoIds = new Set<string>();
  const preparedProducts = selectedTargets.includes('products') ? loaded.products.filter((product) => {
    const mapped = mapMongoProductToCurrentSchemaDetailed(product, {
      ...
      categoryIdByMongoId: selectedTargets.includes('categories') ? undefined : existingLookups.categories,
    });
  ...
  const preparedOrders = selectedTargets.includes('orders') ? loaded.orders.filter((order) => {
    const result = mapMongoOrderToCurrentSchema(order, {
      productIdByMongoId: selectedTargets.includes('products') ? validationProductMap : existingLookups.products,
      importNow: new Date(),
    });

    if (!result.row) {
      const unresolvedState = result.warnings.find((warning) => warning.code === 'unresolved_state');
      if (unresolvedState) {
  ...
  if (report.productDiagnostics.priceFallbacks.length > 0) {
    report.notes.push(`${report.productDiagnostics.priceFallbacks.length} products required a safe price fallback.`);
  }

  if (report.orderDiagnostics.skippedForState.length > 0) {
    report.notes.push(`${report.orderDiagnostics.skippedForState.length} orders will be skipped because their wilaya could not be resolved.`);
  }

  if (report.orderDiagnostics.blockedByCart.length > 0) {
    report.notes.push(`${report.orderDiagnostics.blockedByCart.length} orders are blocked by unmatched cart product references.`);
  }
  ...
  for (const order of mongoOrders) {
    ...
    if (!result.row) {
      ...
      if (result.errors.length > 0) {
        throw new Error(`Order ${readMongoId(order._id) ?? '<unknown>'} failed validation during write phase.`);
      }
...
async function resolveTablesToClear(
  ...
  if (dropScope === 'all') {
    ...
    return (result.rows ?? [])
      ...
      .filter((row: { schema?: unknown; name?: unknown }): row is { schema: string; name: string } => typeof row.schema === 'string' && typeof row.name === 'string')
      .map((row: { schema: string; name: string }) => `${quoteIdentifier(row.schema)}.${quoteIdentifier(row.name)}`);
  }
...
async function confirmWrite(options: LegacyImportCliOptions, report: ValidationReport) {
  ...
  try {
    const scopeDetail = options.dropScope === 'custom'
      ? `${options.dropScope} (${options.dropTables.join(', ')})`
      : options.dropScope;
    console.log(`\nWrite summary:`);
    console.log(`- Targets: ${report.targets.join(', ')}`);
    console.log(`- Drop scope: ${scopeDetail}`);
    console.log(`- Orders blocked by missing cart refs: ${report.orderDiagnostics.blockedByCart.length}`);
...
function printReport(report: ValidationReport, options: LegacyImportCliOptions, files: Record<ImportTarget, string>) {
  ...
  for (const target of IMPORT_TARGETS) {
    ...
    console.log(`- ${IMPORT_TARGET_LABELS[target]}: loaded=${counts.loaded} prepared=${counts.prepared} skipped=${counts.skipped}`);
  }
  ...
  console.log(`- Product purchase prices dropped: ${report.productDiagnostics.purchasePriceDropped}`);
  console.log(`- Orders archived by cutoff: ${report.orderDiagnostics.archivedCount}`);
  console.log(`- Orders skipped for unresolved state: ${report.orderDiagnostics.skippedForState.length}`);
  console.log(`- Orders blocked by unmatched cart refs: ${report.orderDiagnostics.blockedByCart.length}`);
