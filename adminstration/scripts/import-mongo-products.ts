...
function extractDbErrorDetails(error: unknown) {
  ...
  const cause = (candidate.cause && typeof candidate.cause === 'object' ? candidate.cause : candidate) as Record<string, unknown>;

  return {
    message: typeof candidate.message === 'string' ? candidate.message : typeof cause.message === 'string' ? cause.message : null,
...
function parseArgs(argv: string[]): ScriptOptions {
  const positionals = argv.filter((arg, index) => !arg.startsWith('--') && !argv[index - 1]?.startsWith('--'));
  const filePath = positionals[0];

  if (!filePath) {
    throw new Error('Usage: tsx scripts/import-mongo-products.ts <mongo-products.json> [--brands <brands.json>] [--categories <categories.json>] [--replace] [--dry-run]');
  }
...
async function main() {
  const { filePath, brandsPath, categoriesPath, replace, dryRun } = parseArgs(process.argv.slice(2));
  ...
  const [mongoProducts, mongoBrands, mongoCategories] = await Promise.all([
    readFile(filePath, 'utf8').then((input) => parseMongoCollectionExport<MongoProductDocument>(input)),
    brandsPath ? readFile(brandsPath, 'utf8').then((input) => parseMongoCollectionExport<MongoBrandDocument>(input)) : Promise.resolve([]),
    categoriesPath ? readFile(categoriesPath, 'utf8').then((input) => parseMongoCollectionExport<MongoCategoryDocument>(input)) : Promise.resolve([]),
  ]);
  const db = getDb();
  ...
  await db.transaction(async (tx) => {
    ...
    const brandIdByMongoId = await importMongoBrands(
      mongoBrands,
      async (row) => {
    ...
    const categoryIdByMongoId = await importMongoCategories(
      mongoCategories,
      async (row) => {
    ...
    if (productRows.length > 0) {
      ...
      for (let index = 0; index < productRows.length; index += chunkSize) {
        ...
        try {
          ...
          console.error(`Chunk insert failed for products ${index + 1}-${index + chunk.length}: ${summarizeInsertError(error)}`);
          ...
          for (const row of chunk) {
            try {
              ...
              throw new Error(`Failed importing product "${row.title}" (${row.mongoId ?? 'no-mongo-id'}): ${summarizeInsertError(rowError)}`);
            }
          }
        }
        logProgress('Products imported', Math.min(index + chunk.length, productRows.length), productRows.length);
      }
    }
  ...
  console.log('Note: dependent rows with ON DELETE CASCADE/SET NULL were affected by the replacement.');
}
