...
const mocks = vi.hoisted(() => ({
  aiAccess: vi.fn(), mutationAccess: vi.fn(), hasDb: vi.fn(), auth: vi.fn(), review: vi.fn(), reviewAdmin: vi.fn(), proposalRow: vi.fn(),
  revalidateTags: vi.fn(), revalidateProducts: vi.fn(), refreshFeed: vi.fn(),
}));
vi.mock('../../../../../lib/rbac', () => ({ requireAiAccess: mocks.aiAccess, requireMutationAccess: mocks.mutationAccess }));
vi.mock('../../../../../db/client', () => ({ hasDb: mocks.hasDb, getDb: () => ({ select: () => ({ from: () => ({ where: () => ({ limit: mocks.proposalRow }) }) }) }) }));
...
vi.mock('../../../../../lib/ai-admin-capabilities', () => ({ AiAdminCapabilityError: class AiAdminCapabilityError extends Error {}, reviewAdminProposal: mocks.reviewAdmin }));
vi.mock('../../../../../lib/background-jobs', () => ({ startProductCatalogFeedRefreshJob: mocks.refreshFeed }));
vi.mock('../../../../../lib/server-cache', () => ({ CACHE_TAGS: { products: 'products', productsMeta: 'products-meta' }, revalidateServerTags: mocks.revalidateTags }));
vi.mock('../../../../../lib/storefront-revalidate', () => ({ revalidateStorefrontProducts: mocks.revalidateProducts }));

import { PATCH } from './route';

const request = (action: string) => new NextRequest('http://localhost/api/ai/proposals/4', {
  method: 'PATCH', body: JSON.stringify({ action }), headers: { 'content-type': 'application/json' },
});
...
describe('AI proposal review route', () => {
  ...
  it('uses asset access for a featured-products proposal', async () => {
    mocks.proposalRow.mockResolvedValue([{ type: 'featured_products', entityType: 'featured_product_groups' }]);
  ...
  it('requires AI apply and normal product-write access for approval', async () => {
    mocks.mutationAccess.mockResolvedValue(NextResponse.json({ error: 'Forbidden' }, { status: 403 }));
    const response = await PATCH(request('approve'), { params: Promise.resolve({ id: '4' }) });
