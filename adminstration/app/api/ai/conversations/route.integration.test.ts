...
describe('GET /api/ai/conversations', () => {
  ...
  it('returns the signed-in admin account conversations', async () => {
    mocks.rows = [{ id: 4, sessionKey: 'key-4', title: 'Catalog gaps', createdAt: new Date(), updatedAt: new Date() }];

    const response = await GET();

    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ conversations: [expect.objectContaining({ id: 4, title: 'Catalog gaps' })] });
  });
});
