...
const mocks = vi.hoisted(() => ({
  ...
  streamOptions: null as null | { tools?: Record<string, { execute?: (input: unknown) => unknown }> },
...
vi.mock('../../../../lib/auth', () => ({
  auth: async () => ({ user: { email: 'admin@bricomaitre.com', name: 'Admin', permissions: mocks.permissions } }),
}));
...
vi.mock('../../../../lib/ai-background-jobs', () => ({
  ADMIN_BACKGROUND_JOB_TYPES: [
    ...
    'order_export', 'order_ecotrack', 'stats_import', 'ad_cost_import',
    'reporting_refresh', 'ecotrack_catalog_sync', 'ecotrack_shipment_sync', 'storefront_analytics',
  ],
  STARTABLE_ADMIN_BACKGROUND_JOB_TYPES: [
    'product_export', 'catalog_feed_refresh', 'order_export', 'reporting_refresh',
...
describe('POST /api/ai/chat telemetry', () => {
  beforeEach(() => {
    ...
    mocks.startCategorization.mockReset().mockResolvedValue({ kind: 'started', job: { id: 'job-1' } });
    mocks.startContent.mockReset().mockResolvedValue({ kind: 'started', job: { id: 'content-job-1' } });
    mocks.proposeContent.mockReset().mockImplementation(async ({ productId }: { productId: number }) => ({
      id: productId,
      status: 'proposed',
    }));
    mocks.getCategorizationStatus.mockReset().mockResolvedValue({
      id: 'job-1', queue: 'admin-ai-categorization', kind: 'ai-product-categorization', status: 'running',
      progress: { phase: 'classifying-products', current: 12, total: 100, percentage: 12 },
      errorMessage: null, resultSummary: null,
    });
    mocks.listJobs.mockReset().mockResolvedValue([]);
    mocks.getJob.mockReset().mockResolvedValue({ id: 'job-1', status: 'running' });
    mocks.startJob.mockReset().mockResolvedValue({ kind: 'started', job: { id: 'job-2', status: 'queued' } });
    mocks.cancelJob.mockReset().mockResolvedValue({ job: { id: 'job-1', status: 'running', cancelRequested: true } });
  ...
  it('exposes taxonomy lookup and reviewable mutation tools to catalog proposers', async () => {
    ...
    expect(Object.keys(mocks.streamOptions?.tools ?? {})).toEqual(expect.arrayContaining([
      'find_products',
      'find_brands',
      'find_categories',
      'categorize_catalog',
    ...
    expect(mocks.getCategorizationStatus).toHaveBeenCalledWith('admin-ai-categorization', 'admin@bricomaitre.com');
  });
  ...
  it('passes batch auto-apply only when the requester has both apply and product-write permissions', async () => {
    ...
    expect(mocks.startCategorization).toHaveBeenCalledWith('admin@bricomaitre.com', expect.objectContaining({
      autoApply: true,
    }));
  });
  ...
  it('runs at most 20 explicit content proposals inline and queues larger scopes', async () => {
    ...
    expect(mocks.startContent).toHaveBeenCalledWith('admin@bricomaitre.com', expect.objectContaining({
      productIds: Array.from({ length: 21 }, (_, index) => index + 1),
      fields: ['titleAr'],
      onlyMissing: false,
      autoApply: false,
  ...
  it('exposes database-wide job inspection and explicit controls only to settings managers', async () => {
    ...
    await mocks.streamOptions?.tools?.get_background_job?.execute?.({ type: 'ai_categorization', jobId });
    await mocks.streamOptions?.tools?.start_background_job?.execute?.({
      type: 'order_export',
      orderMode: 'selected',
      orderIds: [10, 20],
    });
    await mocks.streamOptions?.tools?.stop_background_job?.execute?.({ type: 'ai_categorization', jobId });
