...
const mocks = vi.hoisted(() => ({ access: vi.fn(), hasDb: vi.fn(), auth: vi.fn(), propose: vi.fn() }));
vi.mock('../../../../../../../lib/rbac', () => ({ requireAiAccess: mocks.access }));
vi.mock('../../../../../../../db/client', () => ({ hasDb: mocks.hasDb }));
...
describe('AI product content proposal route', () => {
  ...
  const request = (body: unknown) => new NextRequest('http://localhost/api/ai/products/1/content/propose', {
  ...
  it('creates a missing-content proposal without changing the product', async () => {
    const response = await POST(request({ fields: ['titleAr', 'descriptionAr'] }), { params: Promise.resolve({ id: '1' }) });
    expect(response.status).toBe(201);
    expect(mocks.propose).toHaveBeenCalledWith({
      productId: 1, fields: ['titleAr', 'descriptionAr'], adminContext: undefined, actorId: 'admin@example.com',
    });
  });

  it('rejects unsupported product fields', async () => {
    const response = await POST(request({ fields: ['price'] }), { params: Promise.resolve({ id: '1' }) });
