...
import { AiContentNotFoundError, AiProposalConflictError, proposeProductContent } from '../../../../../../../lib/ai-product-content';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const proposal = await proposeProductContent({ productId, fields: parsed.data.fields, adminContext: parsed.data.context, actorId: session?.user?.email ?? null });
