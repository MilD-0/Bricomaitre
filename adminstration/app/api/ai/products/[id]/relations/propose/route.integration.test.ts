...
describe('admin product relation AI proposal route', () => {
  ...
  it('enforces the dedicated AI proposal permission', async () => {
    mocks.requireAiAccess.mockResolvedValue(NextResponse.json({ error: 'Forbidden' }, { status: 403 }));
  ...
  it('rejects malformed proposal requests', async () => {
    const response = await POST(request({ targetProductId: 0 }), { params: Promise.resolve({ id: '1' }) });
  ...
  it('rejects self-relations before spending model tokens', async () => {
    const response = await POST(request({ targetProductId: 1 }), { params: Promise.resolve({ id: '1' }) });
