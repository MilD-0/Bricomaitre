...
vi.mock('../../../../../lib/auth', () => ({ auth: async () => ({ user: { email: 'admin@example.com', permissions: mocks.permissions } }) }));
...
describe('POST /api/ai/jobs/cancel', () => {
  beforeEach(() => {
    ...
    mocks.cancelExact.mockReset().mockResolvedValue({ job: { id: 'job-2', status: 'running', cancelRequested: true } });
