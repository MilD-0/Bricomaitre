...
describe('app/api/assets/route', () => {
  ...
  it('returns 400 for invalid featured group payloads', async () => {
    ...
    await expect(res.json()).resolves.toEqual({ error: { fieldErrors: { productIds: ['invalid'] } } });
  });
  ...
  it('creates a banner via action history with assets RBAC', async () => {
    ...
    const valuesMock = vi.fn().mockReturnValue({ returning: vi.fn().mockResolvedValue([{ id: 77 }]) });
