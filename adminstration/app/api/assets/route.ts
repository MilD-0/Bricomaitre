...
async function getNextSortOrder(
  db: Database,
  column: typeof assetBanners.sortOrder | typeof featuredProductGroups.sortOrder | typeof productCards.sortOrder,
  table: typeof assetBanners | typeof featuredProductGroups | typeof productCards,
) {
  const [row] = await db.select({ value: sql<number>`coalesce(max(${column}), -1) + 1` }).from(table);
  return row?.value ?? 0;
}
...
export async function POST(req: NextRequest) {
  ...
  if (body.kind === 'featuredGroup') {
    ...
    const sortOrder = await getNextSortOrder(db, featuredProductGroups.sortOrder, featuredProductGroups);
    await mutateEntityWithHistory(db, {
