...
export async function GET() {
  ...
  const [posts, tagRows, attachmentRows, replyRows, postReactionRows, replyReactionRows, tags] = await Promise.all([
    db.select().from(bulletinPosts).orderBy(desc(bulletinPosts.pinned), desc(bulletinPosts.updatedAt)),
    db
      .select({
  ...
  return NextResponse.json({
    posts: mapBulletinPosts(posts, tagRows, attachmentRows, replyRows, postReactionRows, replyReactionRows, viewer),
    availableTags: tags,
    currentUserId: session.user.id ?? null,
    permissions: {
...
export async function POST(req: NextRequest) {
  ...
  const parsed = bulletinPostSchema.safeParse(await req.json());
  if (!parsed.success) {
