...
import { bulletinPostAttachments, bulletinPostTags, bulletinPosts } from '../../../../db/schema';
import {
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = bulletinPostPatchSchema.safeParse(await req.json());
  if (!parsed.success) {
  ...
  await mutateEntityWithHistory(db, {
    ...
    execute: async (tx) => {
      ...
      if (values.pinned !== undefined) {
        update.pinned = canModerateBulletin(viewer.permissions) ? values.pinned : post.authorId === session.user.id ? values.pinned : post.pinned;
      }
