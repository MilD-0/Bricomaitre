...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const mode = body?.mode === 'confirmed' ? 'confirmed' : body?.mode === 'selected' ? 'selected' : null;
    const rawOrderIds: unknown[] = Array.isArray(body?.orderIds) ? body.orderIds : [];
    const orderIds: number[] = [...new Set(
      rawOrderIds
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    if (!job) {
      return NextResponse.json({ error: 'No order export is currently running.' }, { status: 404, headers: withRequestIdHeaders(requestId) });
    }
