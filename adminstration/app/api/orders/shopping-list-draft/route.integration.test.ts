...
describe('app/api/orders/shopping-list-draft/route', () => {
  ...
  it('returns RBAC denial when access is forbidden', async () => {
    ...
    const response = await GET(new NextRequest('http://localhost/api/orders/shopping-list-draft?sourceMode=confirmed'));
  ...
  it('validates PUT payloads', async () => {
    const response = await PUT(new NextRequest('http://localhost/api/orders/shopping-list-draft', {
      method: 'PUT',
      body: JSON.stringify({ sourceMode: 'confirmed', orderIds: [], title: '', draftItems: [], generatedItems: [], orders: [] }),
  ...
  it('deletes a draft by scope', async () => {
    ...
    const response = await DELETE(new NextRequest('http://localhost/api/orders/shopping-list-draft?sourceMode=selected&orderIds=32&orderIds=31'));
