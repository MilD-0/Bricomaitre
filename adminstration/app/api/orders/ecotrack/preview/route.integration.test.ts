...
const { hasDbMock, getDbMock, requireMutationAccessMock, buildEcotrackPostingPreviewMock } = vi.hoisted(() => ({
  hasDbMock: vi.fn(),
  getDbMock: vi.fn(),
...
describe('app/api/orders/ecotrack/preview/route', () => {
  ...
  it('returns RBAC denial', async () => {
    ...
    const response = await POST(new NextRequest('http://localhost/api/orders/ecotrack/preview', {
      method: 'POST',
  ...
  it('returns preview counts', async () => {
    ...
    expect(buildEcotrackPostingPreviewMock).toHaveBeenCalledWith({ db: true }, 'selected', [11, 12]);
