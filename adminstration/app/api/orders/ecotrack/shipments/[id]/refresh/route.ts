...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const item = await refreshEcotrackOrder(Number(id), {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Unable to refresh ECOTRACK shipment.' }, {
      status: 400,
