...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const label = await fetchSingleEcotrackLabel(Number(id));
    if (!label) {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Unable to fetch the ECOTRACK label.' }, {
      status: 400,
