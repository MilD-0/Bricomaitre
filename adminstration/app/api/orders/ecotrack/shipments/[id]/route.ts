...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const item = await updatePostedEcotrackOrder(await readOrderId(params), parsed, {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Unable to update ECOTRACK shipment.' }, {
      status: 400,
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await deletePostedEcotrackOrder(await readOrderId(params), {
      email: session?.user?.email ?? null,
      name: session?.user?.name ?? null,
    });
    if (!result) {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Unable to delete ECOTRACK shipment.' }, {
      status: 400,
      headers: withRequestIdHeaders(requestId),
    });
  }
