...
describe('app/api/orders/ecotrack/shipments/labels/route', () => {
  ...
  it('returns structured partial label results as JSON', async () => {
    ...
    const response = await POST(new NextRequest('http://localhost/api/orders/ecotrack/shipments/labels', {
      method: 'POST',
  ...
  it('returns structured all-failed label results as JSON', async () => {
    fetchMergedEcotrackLabelsMock.mockResolvedValue({
      ok: false,
      items: [],
      failures: [{ orderId: 11, reference: '11', trackingNumber: 'TRK-11', message: 'Order #11 failed.' }],
      successCount: 0,
      failureCount: 1,
      totalRequested: 1,
      fileName: null,
