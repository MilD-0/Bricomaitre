...
describe('app/api/products/export-all/route', () => {
  ...
  it('returns 401 when no session is available', async () => {
    ...
    const response = await POST(new Request('http://localhost/api/products/export-all', { method: 'POST' }));
  ...
  it('returns 404 when there is no running export to cancel', async () => {
    ...
    await expect(response.json()).resolves.toEqual({ error: 'No export job is currently running.' });
  });
});
