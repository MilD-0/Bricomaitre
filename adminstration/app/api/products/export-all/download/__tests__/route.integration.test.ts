...
describe('app/api/products/export-all/download/route', () => {
  beforeEach(() => {
    ...
    authMock.mockResolvedValue({ user: { id: 'user-1', email: 'admin@example.com', role: 'admin' } });
  ...
  it('returns 400 when jobId is missing', async () => {
    const response = await GET(new NextRequest('http://localhost/api/products/export-all/download'));
  ...
  it('returns 403 for non privileged roles', async () => {
    ...
    const response = await GET(new NextRequest('http://localhost/api/products/export-all/download?jobId=job-1'));
