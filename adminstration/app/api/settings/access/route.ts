...
import { roleDefinitions, userAccessGrants } from '../../../../db/schema';
...
async function loadAccessGrants() {
  ...
  const [grants, customRoles] = await Promise.all([
    ...
    db.select({ id: roleDefinitions.id, name: roleDefinitions.name }).from(roleDefinitions).orderBy(asc(roleDefinitions.name)),
  ]);
...
export async function POST(req: NextRequest) {
  ...
  const parsed = userAccessGrantFormSchema.safeParse(await req.json());
  if (!parsed.success) {
  ...
  if (isReservedPrivilegedEmail(email)) {
    return NextResponse.json({ error: 'Privileged bootstrap emails are managed in code.' }, { status: 409 });
  }
  ...
  await mutateEntityWithHistory(db, {
    ...
    operation: 'create',
    actor,
    execute: (tx) => tx.insert(userAccessGrants).values({
      email,
