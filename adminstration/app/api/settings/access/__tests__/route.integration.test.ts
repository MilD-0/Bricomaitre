...
describe('app/api/settings/access/route', () => {
  ...
  it('creates a new access grant', async () => {
    ...
    const res = await POST(new NextRequest('http://localhost/api/settings/access', {
      method: 'POST',
      body: JSON.stringify({ email: 'Employee@Example.com', role: 'employee', roleDefinitionId: null }),
