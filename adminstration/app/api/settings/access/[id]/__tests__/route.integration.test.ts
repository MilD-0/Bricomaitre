...
describe('app/api/settings/access/[id]/route', () => {
  ...
  it('updates an existing access grant', async () => {
    ...
    const res = await PUT(new NextRequest('http://localhost/api/settings/access/1', {
      method: 'PUT',
  ...
  it('deletes an existing access grant', async () => {
    ...
    const res = await DELETE(new NextRequest('http://localhost/api/settings/access/1', { method: 'DELETE' }), {
      params: Promise.resolve({ id: '1' }),
    });
