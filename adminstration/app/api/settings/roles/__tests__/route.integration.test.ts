...
describe('app/api/settings/roles/route', () => {
  ...
  it('lists existing custom roles and permission catalog', async () => {
    ...
    const roleOrderBy = vi.fn().mockResolvedValue([{ id: 1, name: 'Support', slug: 'support', description: null, isSystem: false, createdAt: new Date('2026-01-01T00:00:00.000Z'), updatedAt: new Date('2026-01-01T00:00:00.000Z') }]);
    const permissionOrderBy = vi.fn().mockResolvedValue([{ roleId: 1, permission: 'orders_write' }]);
    const from = vi
      .fn()
  ...
  it('creates a role with assigned permissions', async () => {
    ...
    getDbMock.mockReturnValue(db);
     ...
     const res = await POST(new NextRequest('http://localhost/api/settings/roles', {
       method: 'POST',
       body: JSON.stringify({ name: 'Campaign Manager', description: 'Handles paid media', permissions: ['brands_categories_write', 'settings_manage'] }),
