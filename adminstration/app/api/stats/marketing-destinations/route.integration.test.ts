...
vi.mock('../../../../lib/marketing-diagnostics', () => ({ getMarketingDestinationDiagnostics: mocks.diagnostics }));
...
describe('GET /api/stats/marketing-destinations', () => {
  beforeEach(() => {
    ...
    mocks.diagnostics.mockReset().mockResolvedValue({ destinations: [{ destination: 'google', accepted: 4 }], recentFailures: [] });
  });
  ...
  it('returns per-destination delivery health without payload or customer data', async () => {
    ...
    await expect(response.json()).resolves.toEqual({ data: { destinations: [{ destination: 'google', accepted: 4 }], recentFailures: [] } });
  });
});
