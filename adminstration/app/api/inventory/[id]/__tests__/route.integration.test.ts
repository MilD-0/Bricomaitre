...
describe('app/api/inventory/[id]/route', () => {
  beforeEach(() => {
    ...
    mutateEntityWithHistoryMock.mockResolvedValue([
      { id: 9, title: 'Hammer', inventoryQuantity: 5, barcode: '123', sku: 'HAM-1', inStock: true, availabilityStatus: 'in_stock', updatedAt: new Date('2026-03-21T00:00:00.000Z') },
    ]);
  });
  ...
  it('returns 400 when delta is zero', async () => {
    ...
    const res = await PATCH(new NextRequest('http://localhost/api/inventory/9', {
      method: 'PATCH',
  ...
  it('returns 404 when the product does not exist', async () => {
    hasDbMock.mockReturnValue(true);
    getDbMock.mockReturnValue({ query: { products: { findFirst: vi.fn().mockResolvedValue(undefined) } } });
  ...
  it('records inventory adjustments through action history', async () => {
    ...
    const returningMock = vi.fn().mockResolvedValue([
      { id: 9, title: 'Hammer', inventoryQuantity: 2, barcode: '123', sku: 'HAM-1', inStock: true, availabilityStatus: 'in_stock', updatedAt: new Date('2026-03-21T00:00:00.000Z') },
    ]);
    ...
    await expect(res.json()).resolves.toEqual({
      ok: true,
      item: { id: 9, title: 'Hammer', inventoryQuantity: 5, barcode: '123', sku: 'HAM-1', inStock: true, availabilityStatus: 'in_stock', updatedAt: '2026-03-21T00:00:00.000Z' },
    });
  });
  ...
  it('updates inStock without changing inventory quantity', async () => {
    ...
    const { execute } = mutateEntityWithHistoryMock.mock.calls[0][1];
    const returningMock = vi.fn().mockResolvedValue([
      { id: 9, title: 'Hammer', inventoryQuantity: 0, barcode: '123', sku: 'HAM-1', inStock: true, availabilityStatus: 'out_of_stock', updatedAt: new Date('2026-03-21T00:00:00.000Z') },
    ]);
    const whereMock = vi.fn().mockReturnValue({ returning: returningMock });
    const setMock = vi.fn().mockReturnValue({ where: whereMock });
    const updateMock = vi.fn().mockReturnValue({ set: setMock });
  ...
  it('updates barcode without changing inventory quantity or stock', async () => {
    ...
    getDbMock.mockReturnValue(db);
    mutateEntityWithHistoryMock.mockResolvedValue([
      { id: 9, title: 'Hammer', inventoryQuantity: 4, barcode: 'NEW-123', sku: 'HAM-1', inStock: true, availabilityStatus: 'in_stock', updatedAt: new Date('2026-03-21T00:00:00.000Z') },
    ]);
    ...
    await expect(res.json()).resolves.toEqual({
      ok: true,
      item: { id: 9, title: 'Hammer', inventoryQuantity: 4, barcode: 'NEW-123', sku: 'HAM-1', inStock: true, availabilityStatus: 'in_stock', updatedAt: '2026-03-21T00:00:00.000Z' },
    });
  });
});
