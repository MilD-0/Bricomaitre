...
describe('app/api/inventory/route', () => {
  ...
  it('returns paginated in-stock inventory rows in the default inventory view', async () => {
    ...
    const offsetMock = vi.fn().mockResolvedValue([
      { id: 2, title: 'Drill', inventoryQuantity: 8, barcode: '456', sku: 'DRI-2', inStock: true, availabilityStatus: 'in_stock', updatedAt: new Date('2026-03-22T00:00:00.000Z') },
      { id: 1, title: 'Hammer', inventoryQuantity: 5, barcode: '123', sku: 'HAM-1', inStock: true, availabilityStatus: 'in_stock', updatedAt: new Date('2026-03-21T00:00:00.000Z') },
    ]);
    ...
    await expect(res.json()).resolves.toEqual({
      writable: true,
      items: [
        { id: 2, title: 'Drill', inventoryQuantity: 8, barcode: '456', sku: 'DRI-2', inStock: true, availabilityStatus: 'in_stock', updatedAt: '2026-03-22T00:00:00.000Z' },
        { id: 1, title: 'Hammer', inventoryQuantity: 5, barcode: '123', sku: 'HAM-1', inStock: true, availabilityStatus: 'in_stock', updatedAt: '2026-03-21T00:00:00.000Z' },
      ],
      pagination: {
        page: 2,
        limit: 2,
  ...
  it('returns searched products even when their inventory quantity is zero', async () => {
    ...
    const offsetMock = vi.fn().mockResolvedValue([
      { id: 7, title: 'Wrench', inventoryQuantity: 0, barcode: '777', sku: 'WRE-7', inStock: false, availabilityStatus: 'out_of_stock', updatedAt: new Date('2026-03-23T00:00:00.000Z') },
    ]);
    const limitMock = vi.fn().mockReturnValue({ offset: offsetMock });
    const orderByMock = vi.fn().mockReturnValue({ limit: limitMock });
    const whereMock = vi.fn().mockReturnValue({ orderBy: orderByMock });
    ...
    await expect(res.json()).resolves.toEqual({
      writable: true,
      items: [
        { id: 7, title: 'Wrench', inventoryQuantity: 0, barcode: '777', sku: 'WRE-7', inStock: false, availabilityStatus: 'out_of_stock', updatedAt: '2026-03-23T00:00:00.000Z' },
      ],
      pagination: {
        page: 1,
        limit: 50,
