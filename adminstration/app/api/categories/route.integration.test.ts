...
describe('app/api/categories/route', () => {
  ...
  it('returns paginated categories with parent options when requested', async () => {
    readCategoriesPageMock.mockResolvedValue({
      items: [{ id: '10', name: 'Paint', slug: 'paint', nameAr: 'طلاء', image: null, isActive: true, status: 'active', parentId: '2', parentName: 'Walls', createdAt: '2026-01-01T00:00:00.000Z', updatedAt: '2026-01-02T00:00:00.000Z', createdBy: null, createdByName: null, updatedBy: null, updatedByName: null }],
    ...
    const response = await GET(new NextRequest('http://localhost/api/categories?includeParentOptions=1'));
  ...
  it('creates categories through the dedicated route', async () => {
    const response = await POST(new NextRequest('http://localhost/api/categories', {
      method: 'POST',
      body: JSON.stringify({ name: 'Paint', nameAr: 'طلاء', imageUrl: 'https://cdn.example.com/paint.jpg', parentId: 2 }),
    }));
