...
describe('app/[locale]/(app)/layout', () => {
  ...
  it('prefers the users table image for the sidebar avatar', async () => {
    const ui = await ProtectedLayout({ children: <div>content</div>, params: Promise.resolve({ locale: 'en' }) });
  ...
  it('redirects disallowed users back to the locale root', async () => {
    ...
    await ProtectedLayout({ children: <div>content</div>, params: Promise.resolve({ locale: 'fr' }) });
