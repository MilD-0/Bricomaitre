...
const { authMock, authPanelMock, connectionMock, getTranslationsMock, redirectMock, signInMock } = vi.hoisted(() => ({
  authMock: vi.fn(),
...
describe('LocaleRootPage', () => {
  ...
  it('redirects authenticated users to administration page', async () => {
    authMock.mockResolvedValue({ user: { email: 'admin@example.com', isAllowed: true, permissions: ['settings_manage'], role: 'admin' } });
  ...
  it('renders the same Google login button for authenticated but disallowed users', async () => {
    ...
    expect(authPanelMock).toHaveBeenCalledWith(expect.objectContaining({
