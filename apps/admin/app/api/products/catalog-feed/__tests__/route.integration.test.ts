...
describe('app/api/products/catalog-feed/route', () => {
  ...
  beforeEach(() => {
    ...
    getStableArtifactUrlMock.mockReturnValue(
      'https://cdn.example.com/exports/products/catalog-feed/latest.csv',
    );
  ...
  it('redirects to the stable catalog feed artifact url', async () => {
    ...
    expect(getStableArtifactUrlMock).toHaveBeenCalledWith(
      'exports/products/catalog-feed/latest.csv',
    );
  ...
  it('allows access with the configured token', async () => {
    ...
    const response = await GET(
      new NextRequest('http://localhost/api/products/catalog-feed?token=secret-token'),
    );
