...
describe('app/api/orders/ecotrack/route', () => {
  ...
  it('returns a specific owned job when jobId is provided', async () => {
    ...
    await expect(response.json()).resolves.toEqual({
      job: { id: 'eco-9', status: 'running', ownerKey: 'user-1' },
    });
  });
  ...
  it('starts a posting job', async () => {
    ...
    expect(startOrderEcotrackJobMock).toHaveBeenCalledWith(
      'user-1',
      {
        mode: 'selected',
        orderIds: [11, 12],
  ...
  it('cancels the running job', async () => {
    const response = await DELETE(
      new Request('http://localhost/api/orders/ecotrack', { method: 'DELETE' }),
    );
