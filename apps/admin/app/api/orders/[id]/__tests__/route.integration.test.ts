...
vi.mock('../../../../../lib/ecotrack', () => ({
  ...
  resolveEcotrackDeliveryFee: vi.fn((catalog, wilayaId, deliveryType) => {
    const fee = catalog.serviceFees.find(
      (entry: { serviceType: string; wilayaId: number }) =>
        entry.serviceType === 'livraison' && entry.wilayaId === wilayaId,
    );
...
describe('app/api/orders/[id]/route', () => {
  beforeEach(() => {
    ...
    readEcotrackCatalogMock.mockResolvedValue({
      wilayas: [],
      communes: [],
      serviceFees: [{ serviceType: 'livraison', wilayaId: 31, homeFee: '400', stopDeskFee: '350' }],
      weightFees: [],
      lastSync: null,
    });
  });
  ...
  it('issues an opaque tracking token for an imported order that does not have one', async () => {
    ...
    const response = await POST(
      new NextRequest('http://localhost/api/orders/7', { method: 'POST' }),
      {
        params: Promise.resolve({ id: '7' }),
      },
    );
  ...
  it('returns validation errors for invalid PATCH payloads', async () => {
    ...
    expect(response.status).toBe(400);
    await expect(response.json()).resolves.toEqual({
      error: { fieldErrors: { phoneNumber1: ['required'] } },
    });
  });
  ...
  it('patches an order and records confirmation history', async () => {
    ...
    expect(ensureOrderConfirmedEventForOrderMock).toHaveBeenCalledWith(
      db,
      expect.objectContaining({
        orderId: 7,
        statusHistoryId: 3,
    ...
    expect(ensureMarketingOrderStatusEventsMock).toHaveBeenCalledWith(
      db,
      expect.objectContaining({
        orderId: 7,
        statusHistoryId: 3,
  ...
  it('queues OrderCompleted after a completed status update', async () => {
    ...
    expect(response.status).toBe(200);
    expect(ensureOrderCompletedEventForOrderMock).toHaveBeenCalledWith(
      db,
      expect.objectContaining({
        orderId: 7,
        statusHistoryId: 40,
