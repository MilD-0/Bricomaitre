...
const DEFAULT_ECOTRACK_SYNC_TIMEZONE = 'Africa/Algiers';
export const DEFAULT_ECOTRACK_SHIPMENT_SYNC_CRON = '*/15 * * * *';
const DEFAULT_ECOTRACK_SHIPMENT_SYNC_TIMEZONE = 'Africa/Algiers';
...
async function runEcotrackShipmentSyncJob(trigger = 'schedule') {
  const state = getSchedulerState();
...
export function startEcotrackScheduler() {
  ...
  const shipmentExpression = (
    process.env.ECOTRACK_ORDER_SYNC_CRON ?? DEFAULT_ECOTRACK_SHIPMENT_SYNC_CRON
  ).trim();
  const shipmentTimezone = (
    process.env.ECOTRACK_ORDER_SYNC_TIMEZONE ?? DEFAULT_ECOTRACK_SHIPMENT_SYNC_TIMEZONE
  ).trim();
  ...
  state.shipmentTask = cron.schedule(
    shipmentExpression,
    () => {
...
export function stopEcotrackScheduler() {
  const state = getSchedulerState();
  state.task?.stop();
