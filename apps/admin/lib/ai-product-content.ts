...
export async function proposeProductContent(input: {
  ...
  if (fields.length === 0)
    throw new AiProposalConflictError('This product has no missing content fields.');
  ...
  const [run] = await db
    ...
    .values({
      ...
      status: 'running',
      model,
      promptVersion: PRODUCT_CONTENT_PROMPT_VERSION,
  ...
  try {
    ...
    if (persistedProposalValuesMatch(product, generated.changes)) {
      throw new AiProposalConflictError(
        'The generated content does not change the current product.',
      );
    }
    const now = new Date();
    ...
    return {
      ...
      before: contentSnapshot(product),
      reasoning: generated.reasoning,
    };
  } catch (error) {
    await db
...
export async function listProductContentProposals(productId: number) {
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      changes: productContentChangesSchema.parse(payload.changes),
...
export async function reviewProductContentProposal(input: {
  proposalId: number;
  ...
  if (input.action === 'reject') {
    const [rejected] = await db
      ...
      .set({
        status: 'rejected',
        reviewedBy: input.actor.email ?? null,
        reviewedAt: new Date(),
        updatedAt: new Date(),
      })
