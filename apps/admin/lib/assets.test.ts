...
const banner = {
  ...
  imageUrlPortrait: 'https://cdn.example.com/portrait.jpg',
  productId: 12,
  active: true,
};
...
describe('banner asset validation', () => {
  ...
  it('keeps the landscape asset as the legacy fallback image', () => {
    expect(assetBannerSchema.parse(banner)).toMatchObject({
      imageUrl: banner.imageUrlLandscape,
      imageUrlPortrait: banner.imageUrlPortrait,
    });
  });
});
