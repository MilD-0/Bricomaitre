...
function resolveWilayaLabel(catalog: EcotrackCatalogExportData | undefined, state: number | null) {
  if (state === null) {
    return '';
  }
...
function resolveCommuneLabel(
  ...
  const commune = catalog.communes.find(
    (entry) =>
      entry.wilayaId === state && (String(entry.communeId) === rawCity || entry.name === rawCity),
  );
