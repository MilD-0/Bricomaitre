...
import { fetchEcotrackCatalogSnapshot } from './ecotrack-catalog/fetch';
import { validateEcotrackToken } from './ecotrack-posting/preview';
...
describe('lib/ecotrack', () => {
  ...
  it('cleans malformed env values', () => {
    expect(cleanEcotrackEnvValue(' "https://example.com/api/v1", ')).toBe(
  ...
  it('normalizes the ECOTRACK catalog snapshot', async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify([{ wilaya_id: 16, wilaya_name: 'Alger' }]), {
          headers: {
  ...
  it('rejects invalid commune payloads', async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify([{ nom: 'Bab Ezzouar' }]), {
          headers: { 'content-type': 'application/json' },
        }),
      )
      .mockResolvedValueOnce(
