...
export function ManualOrderHistory() {
  ...
  const historyQuery = useQuery({
    ...
    queryFn: () =>
      request<ManualOrderHistoryResponse>(`/api/stats/manual-order?page=${page}&limit=10`),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error, _, context) => {
      toast.error(error instanceof Error ? error.message : t('notifications.delete.error'), {
        id: context?.toastId,
      });
    },
  });
              ...
              <TableRow key={row.id}>
                ...
                <TableCell>
                  <div>
                    ...
                    <p
                      className={
                        row.profit >= 0 ? 'text-emerald-600 text-xs' : 'text-rose-600 text-xs'
                      }
                    >
