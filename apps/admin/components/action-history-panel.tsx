...
type ActionHistoryResourceFilter =
  | 'all'
  | 'products'
  | 'orders'
  | 'assets'
    ...
    <Dialog open={Boolean(item)} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-2xl border border-border/70 bg-muted/20 p-4">
            <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
              {t('history.columns.where')}
            </p>
            <p className="mt-2 font-medium text-foreground">
              {t(`history.resources.${item.resource}`)}
            </p>
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              <h3 className="text-sm font-semibold text-foreground">
                {t('history.details.changesTitle')}
              </h3>
              <p className="text-sm text-muted-foreground">
                {t('history.details.changesDescription')}
              </p>
            </div>
            <Badge variant="outline" className="rounded-full">
...
export function ActionHistoryPanel({
  ...
  >({
    ...
    onMutate: ({ label }) => ({
      toastId: toast.loading(t('history.notifications.undo.loading', { entity: label })),
    }),
    onError: (error, variables, context) => {
      toast.error(
        error.message || t('history.notifications.undo.error', { entity: variables.label }),
        { id: context?.toastId },
      );
    },
    onSuccess: async (_data, variables, context) => {
      toast.success(t('history.notifications.undo.success', { entity: variables.label }), {
        id: context?.toastId,
      });
  ...
  >({
    ...
    onMutate: ({ label }) => ({
      toastId: toast.loading(t('history.notifications.redo.loading', { entity: label })),
    }),
    onError: (error, variables, context) => {
      toast.error(
        error.message || t('history.notifications.redo.error', { entity: variables.label }),
        { id: context?.toastId },
      );
    },
    onSuccess: async (_data, variables, context) => {
      toast.success(t('history.notifications.redo.success', { entity: variables.label }), {
        id: context?.toastId,
      });
      await queryClient.invalidateQueries({ queryKey: ['action-history'] });
      await Promise.all(
  ...
  return (
    <section className="overflow-hidden rounded-[1.75rem] border border-border/70 bg-background/95 shadow-sm">
            ...
            <label className="flex min-w-fit items-center gap-3 rounded-lg border border-border/70 bg-background px-3 py-2 text-sm">
              ...
              <span className="flex flex-col">
                <span className="font-medium text-foreground">
                  {t('history.filters.includeEcotrackSyncLabel')}
                </span>
                <span className="text-xs text-muted-foreground">
                  {t('history.filters.includeEcotrackSyncDescription')}
                </span>
              </span>
            </label>
          </div>
      ...
      <div
        className="relative"
        aria-busy={historyQuery.isFetching && historyQuery.data.items.length > 0}
      >
        <div
          className={
            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  <TableHead>
                    <MultiSortHeader
                      label={t('history.columns.action')}
                      sortState={getSortRuleState(sortRules, 'operation')}
                      onClick={() => toggleSort('operation')}
                    />
                  </TableHead>
                  <TableHead>
                    <MultiSortHeader
                      label={t('history.columns.where')}
                      sortState={getSortRuleState(sortRules, 'resource')}
                      onClick={() => toggleSort('resource')}
                    />
                  </TableHead>
                  <TableHead>
                    <MultiSortHeader
                      label={t('history.columns.who')}
                      sortState={getSortRuleState(sortRules, 'createdBy')}
                  ...
                  <TableHead className="w-[240px]">
                    <MultiSortHeader
                      label={t('history.columns.actions')}
                      sortState={getSortRuleState(sortRules, 'isUndone')}
                      onClick={() => toggleSort('isUndone')}
                    />
                  </TableHead>
                </TableRow>
              </TableHeader>
                    ...
                    <TableRow key={item.id}>
                      ...
                      <TableCell>
                        <div className="min-w-[10rem]">
                          ...
                          <p className="mt-1 text-sm text-muted-foreground">
                            {t('history.resourceContext', { entity: item.entityType })}
                          </p>
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="flex min-w-[15rem] flex-wrap items-center justify-end gap-2">
                          <Badge
                            variant={item.isUndone ? 'outline' : 'secondary'}
                          ...
                          <Button
                            ...
                            onClick={() =>
                              undoMutation.mutate({ id: item.id, label: item.entityLabel })
                            }
                          >
                          ...
                          <Button
                            ...
                            onClick={() =>
                              redoMutation.mutate({ id: item.id, label: item.entityLabel })
                            }
                          >
