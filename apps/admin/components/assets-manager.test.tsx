...
describe('AssetsManager', () => {
  ...
  it('creates a banner from the dialog flow', async () => {
    ...
    await userEvent.type(
      within(bannerDialog).getByRole('textbox', { name: 'Arabic title' }),
      'بانر رئيسي',
    );
    ...
    await userEvent.type(
      within(bannerDialog).getByRole('textbox', { name: 'Horizontal screen image' }),
      'https://cdn.example.com/new-banner.jpg',
    );
    await userEvent.type(
      within(bannerDialog).getByRole('textbox', { name: 'Vertical screen image' }),
      'https://cdn.example.com/new-banner-portrait.jpg',
    );
  ...
  it('shows a visible loading state before the assets payload resolves', async () => {
    ...
    expect(screen.getAllByRole('heading', { name: /banner|group|card/i }).length).toBeGreaterThan(
      0,
    );
  ...
  it('shows a recoverable error state when the assets request fails', async () => {
    server.use(
      ...
      http.get('/api/assets/meta', () =>
        HttpResponse.json({ products: [], brands: [], categories: [] }),
      ),
    );
  ...
  it('edits a product card from the dialog flow', async () => {
    ...
    await userEvent.type(
      within(cardDialog).getByRole('textbox', { name: 'French title' }),
      'Carte mise a jour',
    );
  ...
  it('keeps spaces while typing card characteristics', async () => {
    ...
    expect(
      within(cardDialog).queryByText('Too small: expected array to have >=3 items'),
  ...
  it('shows the characteristics minimum error only after submit', async () => {
    ...
    expect(
      await within(cardDialog).findByText('Too small: expected array to have >=3 items'),
    ).toBeInTheDocument();
  });
  ...
  it('lets featured-group selections be added and removed cleanly before validation', async () => {
    ...
    await userEvent.type(
      within(groupDialog).getByRole('textbox', { name: 'Arabic group name' }),
      'مجموعة فارغة',
    );
    ...
    expect(
      await within(groupDialog).findByText('Select at least one product, brand, or category.'),
    ).toBeInTheDocument();
  ...
  it('creates a featured group with storefront top placement enabled', async () => {
    ...
    await userEvent.type(
      within(groupDialog).getByRole('textbox', { name: 'Arabic group name' }),
      'اختيارات الصفحة الرئيسية',
    );
    await userEvent.type(
      within(groupDialog).getByRole('textbox', { name: 'CTA text' }),
      'Voir Plus',
    );
    await userEvent.type(
      within(groupDialog).getByRole('textbox', { name: 'Arabic CTA text' }),
      'اكتشف المزيد',
    );
    await userEvent.type(
      within(groupDialog).getByRole('textbox', { name: 'CTA link' }),
      '/products?featured=1',
    );
    await userEvent.click(
      within(groupDialog).getByRole('switch', { name: 'Show at top of products page' }),
    );
    await userEvent.click(within(groupDialog).getByRole('button', { name: 'Add Roller' }));
    await userEvent.click(within(groupDialog).getByRole('button', { name: 'Create group' }));
  ...
  it('toggles featured group storefront top placement from the table', async () => {
    ...
    const groupsSection = screen
      .getByText('Featured product groups')
      .closest('section') as HTMLElement;
    const placementSwitch = within(groupsSection).getByRole('switch', {
      name: 'Toggle top placement for Summer carousel',
    });
    ...
    await screen.findByText(
      'Pinned featured group Summer carousel to the top of the products page.',
    );
  });
  ...
  it('persists row reordering across banners, groups, and cards', async () => {
    ...
    const cardsSection = screen.getByText('Product cards').closest('section') as HTMLElement;
    await userEvent.click(
      within(cardsSection).getAllByRole('button', { name: 'Move Roller up' })[1],
    );
