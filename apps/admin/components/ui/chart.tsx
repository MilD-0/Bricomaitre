...
export function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        'min-w-40 rounded-2xl border border-border/70 bg-background/95 p-3 shadow-xl backdrop-blur',
        className,
      )}
    >
      {label !== undefined ? (
...
export function ChartLegendContent({
  ...
  className?: string;
  payload?: Array<{ color?: string; dataKey?: string | number; value?: string | number }>;
}) {
  const { config } = useChart();
  ...
  return (
    <div
      className={cn(
        'flex flex-wrap items-center gap-3 pt-3 text-xs text-muted-foreground',
        className,
      )}
    >
      {payload.map((item) => {
