...
describe('Avatar', () => {
  ...
  it('shows the fallback until the image finishes loading', () => {
    ...
    expect(within(container).getByAltText('Ada Lovelace')).toHaveAttribute(
      'src',
  ...
  it('hides the fallback when the image is already complete on mount', () => {
    ...
    expect(screen.getByAltText('Cached avatar')).toHaveAttribute(
      'src',
      'https://example.com/avatar.png',
    );
  });
  ...
  it('resets to the fallback when the image source changes', () => {
    ...
    expect(screen.getByAltText('Switching avatar')).toHaveAttribute(
      'src',
