...
/* eslint-disable @next/next/no-img-element -- Avatar URLs may be supplied by authenticated identity providers outside the configured image origins. */
...
export function Avatar({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  ...
  return (
    <AvatarContext.Provider value={{ imageStatus, setImageStatus }}>
      <div
        className={cn(
          'relative flex size-10 shrink-0 overflow-hidden rounded-full bg-card shadow-[var(--shadow-vapor)]',
          className,
        )}
        {...props}
      />
