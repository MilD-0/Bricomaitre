...
describe('RoleManagementPanel', () => {
  ...
  it('creates a custom role with selected permissions', async () => {
    ...
    await userEvent.type(
      screen.getByPlaceholderText('settings.rolesManager.namePlaceholder'),
      'Campaign Manager',
    );
    await userEvent.click(
      screen.getByLabelText('settings.permissionLabels.brands_categories_write'),
    );
  ...
  it('loads an existing role into the form for editing', async () => {
    ...
    expect(
      screen.getByRole('button', { name: 'settings.rolesManager.updateAction' }),
    ).toBeInTheDocument();
  });
});
