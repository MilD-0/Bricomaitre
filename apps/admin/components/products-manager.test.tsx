...
describe('ProductsManager', () => {
  ...
  function paginatedProductsResponse(requestUrl: string) {
    ...
    const sortRules = url.searchParams
      .getAll('sort')
      .map((value) => {
    ...
    const effectiveSortRules =
      sortRules.length > 0 ? sortRules : [{ key: 'updatedAt', direction: 'desc' as const }];
    ...
    const filtered = products.filter((product) => {
      ...
      if (imageOrigin === 'external') {
        const hasExternalImage = product.images.some(
          (image) => !image.startsWith('https://cdn.example.com/'),
        );
        if (!hasExternalImage) {
          return false;
        }
      ...
      const target = [product.title, product.sku, product.barcode]
        .filter(Boolean)
        .join(' ')
        .toLowerCase();
  ...
  beforeEach(() => {
    ...
    server.use(
      ...
      http.get('/api/products', ({ request }) =>
        HttpResponse.json(paginatedProductsResponse(request.url)),
      ),
      http.get('/api/products/meta', () =>
        HttpResponse.json({
          ...
          categories: [
            { id: 10, name: 'Tools', parentId: null },
            { id: 20, name: 'Paint', parentId: null },
          ],
        }),
      ),
  ...
  it('opens the popup create flow and persists product draft state', async () => {
    ...
    expect(within(restoredDialog).getByRole('textbox', { name: 'Product name' })).toHaveValue(
      'Nova drill',
    );
  ...
  it('generates, reviews, and applies missing AI product content', async () => {
    ...
    const dialog = await screen.findByRole('dialog');
    expect(
      await within(dialog).findByText('No content proposals awaiting review.'),
    ).toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(within(dialog).getByRole('textbox', { name: 'Arabic name' })).toHaveValue(
        'منتج موجود',
      );
      expect(within(dialog).getByRole('textbox', { name: 'Arabic description' })).toHaveValue(
        'وصف عربي مقترح',
      );
    });
  ...
  it('keeps product content incomplete when the server omits persistence verification', async () => {
    ...
    server.use(
      http.get('/api/ai/products/:id/proposals', () =>
        HttpResponse.json({ proposals: [proposal] }),
      ),
      http.patch('/api/ai/proposals/:id', () =>
        HttpResponse.json({
          proposal: {
    ...
    expect(
      await screen.findByText(
        'The server did not verify the product changes. The proposal remains incomplete.',
      ),
    ).toBeInTheDocument();
    expect(within(dialog).getByRole('button', { name: 'Approve and apply' })).toBeInTheDocument();
    expect(within(dialog).getByRole('textbox', { name: 'Arabic name' })).not.toHaveValue(
      'عنوان مقترح',
    );
  });
  ...
  it('keeps selection and bulk actions working after switching views', async () => {
    ...
    await userEvent.click(
      (await screen.findAllByRole('checkbox', { name: 'Select Existing product' }))[0],
    );
    await userEvent.click(screen.getByRole('button', { name: 'Table' }));
  ...
  it('links product table titles to the storefront product page', async () => {
    ...
    const existingProductLink = (
      await screen.findAllByRole('link', { name: 'Existing product' })
    )[0];
    expect(existingProductLink).toHaveAttribute(
      'href',
      'https://bricomaitre.com/products/existing-product',
    );
  ...
  it('supports hierarchical multi-sort with three-click toggles', async () => {
    ...
    const getVisibleOrder = () =>
      screen
        ...
        .map(
          (row) =>
            within(row).getByText(/Active in stock|Active out of stock|Inactive in stock/)
              .textContent,
        );
    ...
    await waitFor(() => {
      expect(getVisibleOrder().slice(0, 3)).toEqual([
        'Inactive in stock',
        'Active out of stock',
        'Active in stock',
      ]);
  ...
  it('supports row toggle mutations and bulk actions with toasts', async () => {
    ...
    await userEvent.click(
      within(productRow).getByRole('checkbox', { name: 'Select Existing product' }),
    );
  ...
  it('filters products by brand and category', async () => {
    ...
    expect(
      screen.getAllByText(
        (_, element) => (element?.textContent ?? '').replace(/\s+/g, ' ').trim() === 'DZD 20',
      ).length,
    ).toBeGreaterThan(0);
  ...
  it('filters products by external image links', async () => {
    ...
    await userEvent.selectOptions(
      screen.getByRole('combobox', { name: 'Filter by image links' }),
      'external',
    );
  ...
  it('supports direct page navigation for larger product tables', async () => {
    ...
    expect((await screen.findAllByRole('button', { name: 'Product 55' })).length).toBeGreaterThan(
      0,
    );
  ...
  it('previews and exports selected products for the Meta catalog', async () => {
    ...
    expect(
      within(dialog).getByText('https://cdn.example.com/p-1.jpg'),
    ).toBeInTheDocument();
  ...
  it('copies selected product ids to the clipboard', async () => {
    ...
    await userEvent.click(
      within(firstRow).getByRole('checkbox', { name: 'Select Existing product' }),
    );
