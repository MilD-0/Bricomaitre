...
function findProductSummary(order: Pick<OrderRecord, 'orderProducts'>, rawValue: string) {
  ...
  return (
    order.orderProducts.find(
      (product) =>
        product.rawValue === trimmed || (numericId !== null && product.productId === numericId),
    ) ?? null
  );
}
...
export function summarizeEditableProducts(items: EditableOrderProduct[]) {
  return buildOrderProductSummaries(
    items.map((item) => item.rawValue),
    (rawValue) => {
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-border/70 p-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {t('ordersManager.products.selectedDescription')}
          </p>
        </div>
        <div className="flex flex-col gap-3">
              ...
              <Card key={product.rawValue} className="rounded-2xl border border-border/70 p-3">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {t('ordersManager.products.lineTotal')}:{' '}
                ...
                <div className="mt-3 flex items-center justify-between gap-3">
                  <Badge variant="outline">
                    {t('ordersManager.products.quantity', { count: product.quantity })}
                  </Badge>
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      size="sm"
                      variant="outline"
                      onClick={() => onDecreaseQuantity(product.rawValue)}
                    ...
                    <Button
                      ...
                      size="sm"
                      variant="outline"
                      onClick={() => onIncreaseQuantity(product.rawValue)}
                    ...
                    <Button
                      ...
                      size="sm"
                      variant="outline"
                      onClick={() => onRemoveProduct(product.rawValue)}
