...
describe('OrdersManager', () => {
  function paginatedOrdersResponse(items: Array<Record<string, unknown>>, requestUrl: string) {
    ...
    const filtered = items.filter((item) => {
      ...
      if (
        ...
        noAnswerCount !== null &&
        noAnswerCount !== '' &&
        String(item.noAnswerCount) !== noAnswerCount
      ) {
        return false;
      }
    ...
    server.use(
      ...
      http.put('/api/orders/shopping-list-draft', async ({ request }) => {
        ...
        return HttpResponse.json({
          ...
          draft: {
            scopeKey:
              body.sourceMode === 'selected'
                ? `selected:${(body.orderIds as number[]).join(',')}`
                : `status:${body.sourceMode}`,
            ...body,
  ...
  it('renders the passive daily order status overview', async () => {
    server.use(
      http.get('/api/orders', ({ request }) =>
        HttpResponse.json(paginatedOrdersResponse([], request.url)),
      ),
    );
  ...
  it('switches both profit projections from confirmed to posted order transitions', async () => {
    ...
    expect(screen.getByRole('switch', { name: 'projection.basisLabel' })).toHaveAttribute(
      'aria-checked',
      'true',
    );
  ...
  it('defaults to card view, persists table view, and restores it from local storage', async () => {
    ...
    server.use(
      http.get('/api/orders', ({ request }) =>
        HttpResponse.json(paginatedOrdersResponse(items, request.url)),
      ),
    );
  ...
  it('keeps inline order controls available in card view', async () => {
    ...
    expect(
      within(cardView).getAllByRole('button', { name: 'ordersManager.actions.editProducts' })
        .length,
    ).toBeGreaterThan(0);
    expect(within(cardView).getAllByLabelText('ordersManager.phone.label').length).toBeGreaterThan(
      0,
    );
    expect(
      within(cardView).getByRole('combobox', { name: 'ordersManager.columns.status' }),
    ).toBeInTheDocument();
    expect(
      within(cardView).getByRole('combobox', { name: 'ordersManager.address.region' }),
    ).toBeInTheDocument();
    expect(
      within(cardView).getByRole('button', { name: 'ordersManager.notes.save' }),
    ).toBeInTheDocument();
  });
  ...
  it('renders the orders table and filters by search', async () => {
    ...
    await userEvent.type(
      screen.getByPlaceholderText('ordersManager.searchPlaceholder'),
      '0550000002',
    );
  ...
  it('clears stale rows while a filtered refetch is pending, even with initial orders', async () => {
    ...
    const initialOrders = paginatedOrdersResponse(
      items,
      'http://localhost/api/orders?page=1&limit=25',
    );
    ...
    await userEvent.selectOptions(
      screen.getByRole('combobox', { name: 'ordersManager.filters.statusLabel' }),
      '2',
    );
  ...
  it('fetches full order history when opening the history dialog', async () => {
    ...
    server.use(
      http.get('/api/orders', ({ request }) =>
        HttpResponse.json(paginatedOrdersResponse(listItems, request.url)),
      ),
    ...
    await userEvent.click(
      screen.getAllByRole('button', { name: 'ordersManager.history.button' })[0],
    );
  ...
  it('filters no-answer orders by counter', async () => {
    ...
    const noAnswerSelect = await screen.findByRole('combobox', {
      name: 'ordersManager.filters.noAnswerCountLabel',
    });
  ...
  it('saves phone updates and deletes orders with toast feedback', async () => {
    ...
    await userEvent.click(
      screen.getByRole('button', { name: 'ordersManager.actions.deleteOrder' }),
    );
    const dialog = await screen.findByRole('dialog');
    ...
    server.use(
      ...
      http.patch('/api/orders/1', async ({ request }) => {
        ...
        items[0] = {
          ...
          fullName: [body.firstName ?? items[0].firstName, body.lastName ?? items[0].lastName]
            .filter(Boolean)
            .join(' '),
          updatedAt: '2026-03-02T10:00:00.000Z',
        };
  ...
  it('shows a leading zero for stored phone numbers that do not include it', async () => {
    ...
    expect(
      screen
        .getAllByRole('link', { name: 'ordersManager.phone.call' })
        .some((link) => link.getAttribute('href') === 'tel:0550000001'),
    ).toBe(true);
  });
  ...
  it('edits order products from the popup and recalculates totals', async () => {
    ...
    await userEvent.click(
      screen.getAllByRole('button', { name: 'ordersManager.actions.editProducts' })[0],
    );
    ...
    await userEvent.type(
      within(dialog).getByPlaceholderText('ordersManager.products.searchPlaceholder'),
      'lamp',
    );

    expect(await within(dialog).findByText('Lamp')).toBeInTheDocument();
    await userEvent.click(
      within(dialog).getByRole('button', { name: 'ordersManager.products.add' }),
    );
    ...
    await userEvent.click(
      within(dialog).getByRole('button', { name: 'ordersManager.products.decreaseQuantity:Desk' }),
    );
  ...
  it('increments the no-answer counter from the orders UI', async () => {
    ...
    await userEvent.click(
      screen.getAllByRole('button', { name: 'ordersManager.status.increaseNoAnswer:3' })[0],
    );
  ...
  it('hides the street input after switching delivery to office', async () => {
    ...
    expect((await screen.findAllByDisplayValue('Mary Jackson')).length).toBeGreaterThan(0);
    expect(
      screen.getAllByPlaceholderText('ordersManager.placeholders.street').length,
    ).toBeGreaterThan(0);
    ...
    await waitFor(() => {
      expect(toastMock.success).toHaveBeenCalled();
      expect(
        screen.queryByPlaceholderText('ordersManager.placeholders.street'),
  ...
  it('updates the region immediately and switches to the first commune in that wilaya', async () => {
    ...
    expect(toastMock.success).toHaveBeenCalled();
    expect(screen.getAllByText(/DZD/).some((node) => node.textContent?.includes('250.00'))).toBe(
      true,
    );
  ...
  it('renders bulk order action controls', async () => {
    ...
    const statusFilter = await screen.findByRole('combobox', {
      name: 'ordersManager.filters.statusLabel',
    });
    const bulkStatus = await screen.findByRole('combobox', {
      name: 'ordersManager.bulk.statusLabel',
    });
    ...
    expect(
      screen.getByRole('button', { name: 'ordersManager.ecotrack.confirmedAction' }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: 'ordersManager.shoppingList.postedAction' }),
    ).toBeInTheDocument();
    ...
    await userEvent.click(
      (
        await screen.findAllByRole('button', {
          name: 'ordersManager.ecotrack.confirmedAction menu',
        })
      )[0],
    );
    expect(
      screen.getByRole('menuitem', { name: 'ordersManager.export.selectedAction' }),
    ).toBeDisabled();
    expect(
      screen.getByRole('menuitem', { name: 'ordersManager.export.confirmedAction' }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('menuitem', { name: 'ordersManager.ecotrack.selectedAction' }),
    ).toBeDisabled();

    await userEvent.click(
      screen.getByRole('button', { name: 'ordersManager.shoppingList.postedAction menu' }),
    );
    expect(
      screen.getByRole('menuitem', { name: 'ordersManager.shoppingList.selectedAction' }),
    ).toBeDisabled();
    expect(
      screen.getByRole('menuitem', { name: 'ordersManager.shoppingList.confirmedAction' }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('menuitem', { name: 'ordersManager.shoppingList.dispatchedAction' }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('menuitem', { name: 'ordersManager.shoppingList.postedAndConfirmedAction' }),
    ).toBeInTheDocument();
  });
  ...
  it('opens the confirmed ecotrack preview from the split button primary action', async () => {
    ...
    await userEvent.click(
      (await screen.findAllByRole('button', { name: 'ordersManager.ecotrack.confirmedAction' }))[0],
    );
    ...
    expect(
      screen.getByRole('button', { name: 'ordersManager.ecotrack.confirm' }),
    ).toBeInTheDocument();
  });
  ...
  it('builds a confirmed shopping list draft, supports edits, and prints the edited draft', async () => {
    ...
    server.use(
      ...
      http.get('/api/products/1', () =>
        HttpResponse.json({
          item: { id: 1, inventoryQuantity: 5, price: 1000, purchasePrice: 700 },
        }),
      ),
      http.get('/api/products/2', () =>
        HttpResponse.json({
          item: { id: 2, inventoryQuantity: 0, price: 1500, purchasePrice: 1000 },
        }),
      ),
    ...
    await userEvent.click(
      (
        await screen.findAllByRole('button', {
          name: 'ordersManager.shoppingList.postedAction menu',
        })
      )[0],
    );
    await userEvent.click(
    ...
    expect(
      screen.getAllByText(
        /ordersManager\.shoppingList\.unitPrice:.*ordersManager\.shoppingList\.purchasePrice:.*700/,
      ).length,
    ).toBeGreaterThan(0);

    await userEvent.click(
      screen.getByRole('button', { name: 'ordersManager.shoppingList.increaseQuantity:Chair' }),
    );
    expect(screen.getByText('Chair x3')).toBeInTheDocument();
    expect(screen.getAllByText('Acme / Chair x1').length).toBeGreaterThan(0);

    const searchInput = screen.getByPlaceholderText(
      'ordersManager.shoppingList.addProductPlaceholder',
    );
    await userEvent.type(searchInput, 'Lamp');
    expect(
      await screen.findByText(/ordersManager\.shoppingList\.purchasePrice:.*600/),
    ).toBeInTheDocument();
    await userEvent.click(
      await screen.findByRole('button', { name: 'ordersManager.shoppingList.addProductAction' }),
    );
    ...
    expect(writeDocumentSpy).toHaveBeenCalledWith(
      expect.stringContaining('https://cdn.example.com/chair.jpg'),
    );
    ...
    expect(writeDocumentSpy).toHaveBeenCalledWith(expect.stringContaining('Purchase price:'));
  ...
  it('loads shared shopping list drafts, merges new generated items, saves edits, and clears shared reset', async () => {
    ...
    server.use(
      ...
      http.get('/api/brands/10', () => HttpResponse.json({ id: 10, name: 'Globex' })),
      http.get('/api/products/1', () =>
        HttpResponse.json({ item: { id: 1, inventoryQuantity: 5 } }),
      ),
      http.get('/api/products/2', () =>
        HttpResponse.json({ item: { id: 2, inventoryQuantity: 0 } }),
      ),
    );
    ...
    await userEvent.click(
      screen.getByRole('button', { name: 'ordersManager.shoppingList.refresh' }),
    );

    expect(await screen.findByText('Desk x1')).toBeInTheDocument();
    expect(
      screen.getAllByText(/^ordersManager\.shoppingList\.generatedAt:/).length,
    ).toBeGreaterThanOrEqual(2);
    expect(
      screen.getAllByText(
        'ordersManager.shoppingList.generatedAt:ordersManager.shoppingList.previousGeneration',
      ).length,
    ).toBeGreaterThan(0);
    ...
    putBodies.length = 0;
    await userEvent.click(
      screen.getByRole('checkbox', { name: 'ordersManager.shoppingList.toggleItem:Chair' }),
    );
    ...
    expect(
      (putBodies.at(-1)?.draftItems as Array<{ draftId: string; checked: boolean }>).find(
        (item) => item.draftId === '9:1',
      )?.checked,
    ).toBe(false);
    expect(
      (putBodies.at(-1)?.draftItems as Array<{ draftId: string }>).some(
        (item) => item.draftId === 'custom:3',
      ),
    ).toBe(true);
  ...
  it('shows confirmed and dispatched empty-state toasts for generated shopping lists and exports', async () => {
    ...
    expect(toastMock.error).toHaveBeenCalledWith('ordersManager.shoppingList.emptyPosted', {
      id: 'toast-id',
    });
    ...
    expect(toastMock.error).toHaveBeenCalledWith('ordersManager.shoppingList.emptyDispatched', {
      id: 'toast-id',
    });
    ...
    expect(toastMock.error).toHaveBeenCalledWith('ordersManager.export.emptyConfirmed', {
      id: 'toast-id',
    });
  });
  ...
  it('ignores confirmed orders older than a week when preparing the confirmed export', async () => {
    ...
    await userEvent.click(
      await screen.findByRole('button', { name: 'ordersManager.export.confirmAndDispatch' }),
    );
  ...
  it('shows export start failures inside the export modal instead of a toast', async () => {
    ...
    server.use(
      ...
      http.post(
        '/api/orders/export',
        () => new HttpResponse('Export failed hard', { status: 500 }),
      ),
    );
  ...
  it('copies the opaque storefront tracking URL from an order row action', async () => {
    ...
    server.use(
      ...
      http.get('/api/orders/overview', () =>
        HttpResponse.json({
          overview: { available: false, reportDay: null, timezone: 'Africa/Algiers' },
        }),
      ),
      http.get('/api/ecotrack/catalog', () =>
        HttpResponse.json({
      ...
      http.get('/api/orders/export', () => HttpResponse.json({ job: null })),
      http.get('/api/orders/ecotrack', () => HttpResponse.json({ job: null })),
      http.post('/api/orders/52', () =>
        HttpResponse.json({ ok: true, publicToken: 'secure-public-token-1234567890' }),
      ),
    );
    ...
    await userEvent.click(
      (await screen.findAllByRole('button', { name: 'ordersManager.tracking.copy' })).at(-1)!,
    );
