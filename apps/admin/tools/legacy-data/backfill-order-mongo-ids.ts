...
import type { MongoOrderDocument } from './mongo-product-import';
import { readMongoId } from './mongo-product-import';
...
function loadLegacyOrders() {
  ...
  const items = parsed.filter(
    (item): item is MongoOrderDocument => typeof item === 'object' && item !== null,
  );
...
function resolveCandidate(
  ...
  const byTracking = order.ecotrackTrackingNumber
    ? lookups.trackingLookup.get(order.ecotrackTrackingNumber)
    : null;
  const byReference = order.ecotrackReference
    ? lookups.referenceLookup.get(order.ecotrackReference)
    : null;
...
async function main() {
  ...
  for (const item of matches) {
    await db.update(orders).set({ mongoId: item.candidate.mongoId }).where(eq(orders.id, item.id));
  }
