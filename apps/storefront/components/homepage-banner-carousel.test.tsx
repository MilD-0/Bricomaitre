...
const banners = [1, 2].map((id) => ({
  id,
  title: `Banner ${id}`,
  titleAr: null,
  imageUrl: `/banner-${id}.jpg`,
  imageUrlPortrait: `/banner-${id}-portrait.jpg`,
  imageUrlLandscape: `/banner-${id}-wide.jpg`,
  productId: id,
  sortOrder: id - 1,
  active: true,
  createdAt: stamp,
...
describe('HomepageBannerCarousel', () => {
  ...
  it('keeps the carousel DOM direction aligned with the Arabic Embla direction', () => {
    const { container } = render(<HomepageBannerCarousel banners={banners} locale="ar" />);

    expect(container.querySelector('.home-banner-viewport')).toHaveAttribute('dir', 'rtl');
    expect(container.querySelectorAll('.home-banner-slide')).toHaveLength(2);
  });
