...
export function CatalogCard({
  ...
  const highPriorityImage = eagerImage && position <= 2;
    ...
    <article className="catalog-card">
      ...
      >
        <div className="catalog-card-media">
          ...
          {!product.inStock ? (
            <span className="catalog-unavailable-badge">{labels.outOfStock}</span>
          ) : null}
        </div>
        <div className="catalog-card-body">
          ...
          <span
            className={
              product.inStock ? 'catalog-stock catalog-stock-in' : 'catalog-stock catalog-stock-out'
            }
          >
            <i aria-hidden="true" />
          ...
          <div className="catalog-card-footer">
            <div className="catalog-card-price">
              {product.price ? (
                <strong className="catalog-card-current-price">
                  {formatProductPrice(product.price, locale)}
                </strong>
              ) : (
                <strong className="catalog-card-price-request">{labels.priceOnRequest}</strong>
              )}
              {discount > 0 && product.oldPrice ? (
                <del className="catalog-card-compare-price">
