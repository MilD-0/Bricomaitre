...
describe('CatalogCard', () => {
  ...
  it('uses localized RTL copy and exposes unavailability in both the media and product summary', () => {
    const html = renderToStaticMarkup(
      <CatalogCard
        ...
        locale="ar"
        position={5}
        labels={{ ...labels, outOfStock: 'غير متوفر', viewProduct: 'عرض' }}
      />,
    );
  ...
  it('eagerly loads the initial desktop row and leaves later cards lazy', () => {
    const second = renderToStaticMarkup(
      <CatalogCard product={product} locale="fr" position={2} labels={labels} />,
    );
    const fourth = renderToStaticMarkup(
      <CatalogCard product={product} locale="fr" position={4} labels={labels} />,
    );
    const fifth = renderToStaticMarkup(
      <CatalogCard product={product} locale="fr" position={5} labels={labels} />,
    );

    expect(second).toContain('loading="eager"');
    expect(fourth).toContain('loading="eager"');
    expect(fifth).toContain('loading="lazy"');
  });
