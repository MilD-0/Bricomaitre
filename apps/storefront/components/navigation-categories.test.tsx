...
describe('NavigationCategories', () => {
  ...
  it('reserves navigation space while categories are loading', async () => {
    let resolveMeta: (value: {
      categories: Array<{ id: number; name: string; nameAr: string | null; slug: string | null }>;
    ...
    resolveMeta!({
      categories: [{ id: 8, name: 'Éclairage', nameAr: null, slug: 'eclairage' }],
      brands: [{ id: 9, name: 'Wadfow', slug: 'wadfow' }],
    });
    fireEvent.click(await screen.findByRole('button', { name: 'Catégories' }));
    expect(await screen.findByRole('link', { name: 'Éclairage' })).toHaveAttribute(
      'href',
    ...
    await waitFor(() =>
      expect(document.querySelector('.navigation-categories-skeleton')).not.toBeInTheDocument(),
    );
  });
