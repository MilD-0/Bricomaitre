...
describe('SupportContactActions', () => {
  ...
  it('offers a direct call action without tracking the phone number', () => {
    render(
      <SupportContactActions
        ...
        contact={contact}
        surface="checkout"
        labels={{ title: 'Besoin d’aide ?', call: 'Appeler' }}
      />,
    );
    ...
    expect(analytics).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ metadata: { surface: 'checkout', target: 'call' } }),
    );
  ...
  it('keeps phone support visible when a legacy row has the retired flag disabled', () => {
    const { container } = render(
      <SupportContactActions
        locale="fr"
        contact={{ ...contact, phoneEnabled: false }}
        surface="thank_you"
        labels={{ title: 'Support', call: 'Call' }}
      />,
    );
