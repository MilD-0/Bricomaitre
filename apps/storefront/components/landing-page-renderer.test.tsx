...
vi.mock('@/components/product-actions', () => ({
  ProductActions: ({
    ...
    available: boolean;
    analytics: { metadata: unknown };
    buyNowTarget?: string;
  }) => (
    <div
      data-testid="actions"
      data-available={available}
      data-metadata={JSON.stringify(analytics.metadata)}
      data-buy-target={buyNowTarget}
    />
  ),
}));
vi.mock('@/components/storefront-image', () => ({
  StorefrontImage: ({ alt }: { alt: string }) => <span role="img" aria-label={alt} />,
}));
    ...
    blocks: [
      {
        ...
        subheading: 'Une campagne unique.',
        imageUrl: null,
        imageAlt: '',
        primaryCtaLabel: 'Je commande',
        showAddToCart: true,
      },
      {
        id: 'story',
        ...
        heading: 'Une composition éditoriale',
        body: 'Le produit devient le fil conducteur.',
        highlights: ['Direct', 'Lisible'],
      },
      {
        id: 'gallery',
        type: 'image-gallery',
        ...
        images: [
          { imageUrl: null, imageAlt: 'Vue un', caption: 'Face avant' },
          { imageUrl: null, imageAlt: 'Vue deux', caption: 'Vue latérale' },
        ],
      },
      {
        id: 'uses',
        ...
        body: '',
        items: [
          { title: 'Atelier', description: 'Pour les travaux indiqués.', icon: 'tool' },
          { title: 'Entretien', description: 'Pour les usages indiqués.', icon: 'target' },
        ],
      },
      {
        id: 'comparison',
        type: 'comparison',
        variant: 'spotlight',
        heading: 'Décidez clairement',
        productLabel: 'Perceuse',
        ...
        footnote: 'Données vérifiées.',
      },
      {
        id: 'process',
        type: 'process',
        variant: 'timeline',
        heading: 'Trois gestes',
        body: '',
        steps: [
          { title: 'Choisissez', description: 'Sélectionnez.' },
          { title: 'Confirmez', description: 'Répondez à notre appel.' },
        ],
      },
      {
        id: 'trust',
      ...
      },
      {
        ...
        type: 'commerce-panel',
        variant: 'image-led',
        heading: 'Votre perceuse est ici',
        body: 'Prix live.',
        bullets: [],
        imageUrl: null,
        imageAlt: '',
        primaryCtaLabel: 'Je la commande',
        showAddToCart: true,
      },
      {
        id: 'faq',
        ...
        heading: 'Questions',
        items: [{ question: 'Comment commander ?', answer: 'Par le formulaire.' }],
      },
      {
        id: 'final',
...
describe('LandingPageRenderer', () => {
  it('renders governed blocks while keeping live product price and availability authoritative', () => {
    ...
    expect(screen.getAllByTestId('actions')[0]).toHaveAttribute(
      'data-buy-target',
      '#landing-order',
    );
    ...
    expect(screen.getByRole('link', { name: /Je commande.*12/ })).toHaveAttribute(
      'href',
      '#landing-order',
    );
    expect(container.querySelector('script[type="application/ld+json"]')?.textContent).toContain(
      '/fr/landing/perceuse-20v',
    );
  });
});
