...
describe('NavigationActions', () => {
  ...
  it('opens the live cart in a dialog and links onward to checkout', async () => {
    ...
    expect(screen.getByRole('link', { name: labels.cartDrawer.checkout })).toHaveAttribute(
      'href',
      '/fr/checkout',
    );
    expect(analytics).toHaveBeenCalledWith(
      expect.objectContaining({ eventName: 'view_cart', quantity: 3 }),
    );
  });
  ...
  it('opens an accessible drawer with categories grouped beneath their parent and every brand', async () => {
    ...
    const categoryGroup = categoryGroups[0] as HTMLElement;
    expect(
      within(categoryGroup).getByText('Équipement d’atelier', { selector: 'summary span' }),
    ).toBeVisible();
    expect(
      within(categoryGroup).queryByRole('link', { name: /Éclairage/ }),
    ).not.toBeInTheDocument();
    fireEvent.click(categoryGroup.querySelector('summary')!);
    expect(
      await within(categoryGroup).findByRole('link', { name: /Équipement d’atelier/ }),
    ).toHaveAttribute('href', '/fr/categories/equipement-atelier');
    expect(within(categoryGroup).getByRole('link', { name: /Éclairage/ })).toHaveAttribute(
      'href',
      '/fr/categories/eclairage',
    );
    expect(within(drawer).getByRole('link', { name: 'Promos' })).toHaveAttribute(
      'href',
      '/fr/products?discounted=1',
    );
    fireEvent.click(within(drawer).getByText(labels.brands, { selector: 'summary span' }));
    expect(await screen.findByRole('link', { name: /Wadfow/ })).toHaveAttribute(
      'href',
      '/fr/brands/wadfow',
    );
    ...
    expect(within(drawer).getByRole('link', { name: /Appeler.*0795 34 28 26/ })).toHaveAttribute(
      'href',
      'tel:+213795342826',
    );
    expect(analytics).toHaveBeenCalledWith(
