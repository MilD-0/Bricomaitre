...
function getCatalogUrl(locale: Locale) {
...
export function buildTaxonomyCatalogMetadata({
  locale,
  kind,
  name,
  nameAr,
  slug,
  filtered = false,
}: TaxonomyMetadataInput): Metadata {
  const { title: localizedName, description } = buildTaxonomyCatalogCopy({
    locale,
    kind,
    name,
    nameAr,
  ...
  const title = locale === 'ar' ? `${localizedName} | بريكوماتر` : `${localizedName} | Bricomaitre`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        ...Object.fromEntries(
          locales.map((entry) => [entry, `${getStorefrontSiteUrl()}/${entry}${path}`]),
        ),
...
export function buildTaxonomyCatalogCopy({
  locale,
  kind,
  name,
  nameAr,
}: Omit<TaxonomyMetadataInput, 'slug' | 'filtered'>) {
