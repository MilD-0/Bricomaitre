...
export function shouldCaptureServerException(env: Partial<NodeJS.ProcessEnv> = process.env) {
  if (env.NEXT_PHASE === 'phase-production-build') return false;
  return Boolean(normalizeSentryDsn(env.SENTRY_DSN_STOREFRONT));
}

export function captureProductPageException(
  error: unknown,
  context: {
    locale: string;
...
export function captureCatalogPageException(
  error: unknown,
  context: {
    locale: string;
    operation: string;
