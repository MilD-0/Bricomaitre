...
export function buildStorefrontSitemap(
  ...
  taxonomy: { categories: SitemapTaxonomy[]; brands: SitemapTaxonomy[] } = {
    categories: [],
    brands: [],
  },
  ...
  const productEntries: MetadataRoute.Sitemap = [...uniqueProducts.entries()].flatMap(
    ([token, product]) => {
  ...
  const taxonomyEntries: MetadataRoute.Sitemap = (['categories', 'brands'] as const).flatMap(
    (kind) => {
