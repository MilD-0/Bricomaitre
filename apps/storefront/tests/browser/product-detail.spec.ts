...
test('renders the French product journey with SEO and governed analytics', async ({ page }) => {
  ...
  const response = await page.goto(
    '/fr/products/desk-lamp?fbclid=browser-click&utm_source=facebook&utm_medium=paid_social&private=discard-me',
  );
  ...
  const productHeadingWeight = Number(
    await productHeading.evaluate((element) => getComputedStyle(element).fontWeight),
  );
  ...
  expect(
    await page
      .locator('html')
      .evaluate((element) => getComputedStyle(element).getPropertyValue('--brand-teal').trim()),
  ).toBe('#01737a');
  expect(
  ...
  expect(
    brandContainerBox && brandImageBox && brandContainerBox.width - brandImageBox.width,
  ).toBeLessThan(10);
  expect(
    await page
      .locator('.product-brand-logo')
  ...
  const productImages = page.locator('.product-media-zoom-trigger > img');
  ...
  const mediaImageBox = await productImages.first().boundingBox();
  expect(
    mediaStageBox &&
      mediaImageBox &&
      mediaImageBox.x >= mediaStageBox.x &&
      mediaImageBox.y >= mediaStageBox.y,
  ).toBe(true);
  expect(
    mediaStageBox &&
      mediaImageBox &&
      mediaImageBox.x + mediaImageBox.width <= mediaStageBox.x + mediaStageBox.width + 1,
  ).toBe(true);
  expect(
    mediaStageBox &&
      mediaImageBox &&
      mediaImageBox.y + mediaImageBox.height <= mediaStageBox.y + mediaStageBox.height + 1,
  ).toBe(true);
  ...
  expect(await buyNow.evaluate((element) => getComputedStyle(element).backgroundColor)).toBe(
    'rgb(242, 106, 33)',
  );
  expect(
    await page
  ...
  await expect
    .poll(() => firstTrustIcon.evaluate((element) => getComputedStyle(element).transform))
    .not.toBe('none');
  const breadcrumbs = page.getByRole('navigation', { name: 'Fil d’Ariane' });
  await expect(breadcrumbs.getByRole('link', { name: 'Équipement d’atelier' })).toHaveAttribute(
    'href',
    '/fr/categories/workshop-equipment',
  );
  await expect(breadcrumbs.getByRole('link', { name: 'Éclairage' })).toHaveAttribute(
    'href',
    '/fr/categories/lighting',
  );
  ...
  await expect(page.locator('.quantity-control output')).toHaveText('2');
  ...
  await expect(
    similar
      .locator('.similar-products-heading')
      .getByRole('heading', { name: 'Produits similaires' }),
  ).toBeVisible();
  await expect(
    similar.locator('.similar-products-heading p, .similar-products-heading span'),
  ).toHaveCount(0);
  await expect(
    similar.getByRole('heading', { level: 2, name: 'Projecteur de chantier sur trépied' }),
  ).toBeVisible();
...
test('preserves Arabic RTL and narrow-phone usability', async ({ page }) => {
  ...
  expect(await mobileBuyNow.evaluate((element) => getComputedStyle(element).position)).toBe(
    'fixed',
  );
  ...
  expect(mobileActionDock).toEqual({
    position: 'fixed',
    bottom: '0px',
...
test('redirects legacy tokens and distinguishes missing from unavailable products', async ({
  ...
  await expect(
    page.getByRole('heading', { name: 'Impossible de charger ce produit' }),
  ).toBeVisible();
});
...
test('keeps essential product content useful without JavaScript', async ({ browser }) => {
  ...
  await expect(page.getByRole('heading', { level: 1, name: 'Lampe de travail' })).toBeVisible();
  await expect(
    page.getByText('Une lumière stable et puissante', { exact: false }).first(),
  ).toBeVisible();
  await expect(page.locator('.product-summary .availability').getByText('En stock')).toBeVisible();
  await context.close();
});

test('meets the weak-phone first-content budget under CPU and network throttling', async ({
  page,
  context,
}) => {
