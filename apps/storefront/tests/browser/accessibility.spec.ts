...
for (const locale of ['fr', 'ar'] as const) {
  ...
  for (const surface of ['homepage', 'catalog', 'product', 'checkout', 'thank-you'] as const) {
    test(`${surface} has no WCAG A/AA violations in ${locale} ${mobile ? 'mobile' : 'desktop'}`, async ({
      page,
      request,
    }) => {
      await page.setViewportSize(
        mobile ? { width: 360, height: 740 } : { width: 1440, height: 900 },
      );
      ...
      await expect(
        page
          .getByRole('heading', { level: headingLevel, name: readyNames[surface][locale] })
          .first(),
      ).toBeVisible();
