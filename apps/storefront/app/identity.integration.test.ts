...
describe('storefront identity metadata', () => {
  ...
  it('bundles storefront fonts locally instead of requiring Google during compilation', async () => {
    const layout = await readFile(path.join(process.cwd(), 'app/[locale]/layout.tsx'), 'utf8');

    expect(layout).toContain("from 'next/font/local'");
    expect(layout).toContain('@fontsource-variable/inter/files/inter-latin-wght-normal.woff2');
