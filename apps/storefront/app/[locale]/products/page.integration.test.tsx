...
import { CatalogPageContent } from './page-content';
...
vi.mock('next/image', () => ({
  default: (props: Record<string, unknown>) =>
    React.createElement('img', { ...props, fetchPriority: undefined }),
}));
...
vi.mock('@/components/catalog-live-search', () => ({
  CatalogLiveSearch: ({ initialValue }: { initialValue: string }) =>
    React.createElement('input', { name: 'q', value: initialValue, readOnly: true }),
}));
vi.mock('@/components/catalog-live-sort', () => ({
  CatalogLiveSort: ({ initialValue, label }: { initialValue: string; label: string }) =>
...
vi.mock('next-intl/server', () => ({
  getTranslations: vi.fn(
    async () => (key: string, values?: Record<string, number>) =>
      (
        ({
          eyebrow: 'Catalog',
          title: 'Products for your work',
          ...
          sortNewest: 'Newest',
          sortPriceAsc: 'Low price',
          sortPriceDesc: 'High price',
          sortNameAsc: 'Name',
          apply: 'Apply',
          reset: 'Reset',
          inStock: 'In stock',
          ...
          previous: 'Previous',
          next: 'Next',
          results: `${values?.count ?? 0} products`,
...
describe('localized Catalog Page', () => {
  ...
  it('server-renders product discovery, filters, price, and stable product links', async () => {
    ...
    expect(html.match(/<legend>/g)).toHaveLength(4);
    expect(html).not.toContain('Apply</button>');
    expect(mocks.catalog).toHaveBeenCalledWith(
      expect.objectContaining({ search: 'lamp', categoryId: 3, limit: 24 }),
    );
  });
