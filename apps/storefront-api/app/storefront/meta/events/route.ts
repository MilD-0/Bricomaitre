...
export async function POST(request: NextRequest) {
  ...
  if (Number.isFinite(contentLength) && contentLength > MAX_META_BODY_BYTES) {
    return NextResponse.json(
      { error: 'Meta event request is too large.' },
      {
        status: 413,
        headers: withRequestIdHeaders(requestId),
      },
  ...
  if (!rateLimit.ok) {
    return NextResponse.json(
      { error: 'Too many Meta event requests.' },
      {
        status: 429,
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { error: 'Invalid Meta event.', details: parsed.error.flatten() },
      {
        status: 400,
        headers: withRequestIdHeaders(requestId),
      },
  ...
  if (!authorizeMetaSourceRequest(request, parsed.data.eventSourceUrl)) {
    return NextResponse.json(
      { error: 'Meta event source is not allowed.' },
      {
        status: 403,
        headers: withRequestIdHeaders(requestId),
      },
  ...
  await enqueueMetaBrowserEvent(
    getDb(),
    parsed.data,
    getMetaRequestContext(request, parsed.data.eventSourceUrl),
  );
  return NextResponse.json(
    {
      ok: true,
