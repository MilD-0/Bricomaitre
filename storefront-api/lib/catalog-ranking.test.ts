...
describe('recommended catalog ranking', () => {
  it('pins direct, brand, and category featured-group matches before engagement ranking', () => {
    ...
    expect(query).toMatch(/sort_order[\s\S]+in_stock[\s\S]+popularity_score[\s\S]+purchase_count[\s\S]+checkout_count[\s\S]+add_to_cart_count[\s\S]+view_count[\s\S]+conversion_rate[\s\S]+last_viewed_at[\s\S]+updated_at[\s\S]+"products"\."id"/);
  ...
  it('places textual relevance ahead of featured and commerce ranking for a search', () => {
    const dialect = new PgDialect();
    const query = dialect.sqlToQuery(sql.join(buildRecommendedProductOrderBy('perceuse'), sql.raw(', '))).sql;

    expect(query.indexOf('word_similarity')).toBeLessThan(query.indexOf('show_at_top_of_products_page'));
  });
});
