...
describe('GET /storefront/orders/track/:token', () => {
  beforeEach(() => {
    ...
    rateLimitMock.mockResolvedValue({ ok: true, limit: 60, remaining: 59, resetAt: new Date().toISOString() });
  });
  ...
  it('resolves an order using only its opaque public token', async () => {
    ...
    const response = await GET(new NextRequest(`http://localhost/storefront/orders/track/${token}`), {
  ...
  it('does not reveal whether malformed or unknown tokens exist', async () => {
    ...
    const unknown = await GET(new NextRequest(`http://localhost/storefront/orders/track/${token}`), {
      params: Promise.resolve({ token }),
    });
