...
describe('storefront homepage route', () => {
  ...
  it('returns a stable empty shape without a database', async () => {
    ...
    await expect((await GET()).json()).resolves.toEqual({ banners: [], topProducts: [], categories: [], productCards: [], brands: [], featuredGroups: [] });
  });

  it('returns the aggregated homepage and applies shared cache tags', async () => {
    const homepage = { banners: [{ id: 1 }], topProducts: [], categories: [], productCards: [], brands: [], featuredGroups: [] };
    mocks.hasDb.mockReturnValue(true); mocks.getDb.mockReturnValue({ db: true }); mocks.read.mockResolvedValue(homepage);
