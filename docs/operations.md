...
3. starts the candidate Storefront API in the inactive slot and checks its live
   catalog contract;
4. applies migrations, then proves that the previous slot remains healthy and
   rollback-compatible;
5. starts the candidate Storefront, Admin, and Admin worker and waits for their
   readiness checks;
6. refreshes persisted reporting before the release becomes visible;
7. switches Nginx to the candidate and runs public smoke checks across health,
   both locales, catalog, checkout, robots, and sitemap behavior; and
8. starts the analytics worker, records the current and previous verified
...
container. The drill verifies core tables, the migration ledger, and the
restored database size before deleting the temporary container and volume.
...
Web builds publish source maps. Application health routes, worker heartbeats,
